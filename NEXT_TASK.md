## 2026-09-12: Цепочка сборки проверена — двадцать восьмая проверка

- **Готово:** мёртвое второе определение регулярки удалено из `db/regimen_semantics.py`,
  класс дефекта закреплён AST-проверкой. Тесты: **2412 passed**.
- **Что проверено в цепочке сборки:** `db/regimen_semantics.py` (413 строки) прочитан
  целиком; `parse_duration` читает только `group(1)`/`group(0)`, месячные формы закреплены
  существующими тестами; пересборка даёт прежний SHA-256.
- **Если продолжать, остались:** `db/build_db.py` (271 строка) и `db/build_html.py`
  (89 строк) как код; `benchmark_review_workbench.py` и корневые скрипты.
- **Ранее закрыто за день:** двадцать семь проверок, верстак ревью покрыт полностью,
  связь клинреков с калькулятором подтверждена end-to-end на живых серверах.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
  две записи с `freq_per_day: 30`, `netilmicin` и `tobramycin` без `dilution`.
...
## 2026-09-12: Основной результат подтверждён end-to-end

- **Готово:** связь клинреков с калькулятором проверена на живых серверах по обоим
  каналам; `api/service.py` прочитан целиком, дефектов нет. Тесты: **2409 passed**.
- **Что закрыто за день:** двадцать семь проверок, верстак ревью покрыт полностью,
  единый ворклист на все 264 находки валидатора, граница двух валидаторов закреплена.
- **Если продолжать аудит, остались:** `benchmark_review_workbench.py` и корневые
  скрипты; цепочка сборки `db/build_db.py` → `db/regimen_semantics.py` как код, а не
  через тесты.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
  две записи с `freq_per_day: 30`, `netilmicin` и `tobramycin` без `dilution`.
- **Инфраструктурное:** базы KB (`metadata.sqlite`, `normalized_regimens.sqlite`) в
  репозитории отсутствуют, поэтому `/v1/health` отдаёт degraded, а `build_initial_queue`
  невыполним. Это не дефект, а отсутствие данных; для проверки нужны сами базы.
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Верстак ревью закрыт — двадцать седьмая проверка

- **Готово:** происхождение клинических находок защищено, `issue_registry` покрыт
  16 тестами. Тесты: **2409 passed**.
- **Верстак ревью закрыт целиком:** `permissions` удалён; `dose_unit_audit`,
  `pilot_policy`, `priority`, `storage`, `reporting`, `issue_registry` прочитаны и
  покрыты тестами. Модулей без покрытия не осталось.
- **Итог по верстаку:** из пяти проверенных стыков и модулей три дали дефект
  (`missing_dose` в ярусе 5, дубль политики в v1, пустой штамп происхождения), один —
  дубль, два оказались чистыми и закреплены. Дефекты находились на стыках, а не внутри
  модулей.
- **Если продолжать аудит, другие направления:** `benchmark_review_workbench.py` и
  корневые скрипты; `clinical_engine/api/service.py` за пределами роута
  `/v1/guidelines/{disease_id}`; цепочка сборки `db/build_db.py` → `db/regimen_semantics.py`
  как код, а не через тесты.
- **Ранее закрыто за день:** двадцать шесть находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
  две записи с `freq_per_day: 30`, `netilmicin` и `tobramycin` без `dilution`.
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Отчётность закрыта — двадцать шестая проверка

- **Готово:** `reporting.py` проверен и закреплён 12 тестами. Тесты: **2393 passed**.
- **Верстак ревью почти закрыт:** `permissions` удалён, `dose_unit_audit` и
  `pilot_policy` прочитаны и починены, `priority` и `storage` закреплены, `reporting`
  закрыт. Без тестового покрытия остался только `issue_registry` (34 строки, 1 импорт).
- **Общий вывод по верстаку:** дефекты находились не внутри модулей, а на стыках —
  словарь значений, который пишет один модуль и читает другой, отбрасывая неизвестное
  молча. Четыре стыка проверены, один дал дефект, один — дубль политики.
- **Если продолжать аудит, другие направления:** `benchmark_review_workbench.py` и
  корневые скрипты; `clinical_engine/api/service.py` за пределами роута
  `/v1/guidelines/{disease_id}`; цепочка сборки `db/build_db.py` → `db/regimen_semantics.py`
  как код, а не через тесты.
- **Ранее закрыто за день:** двадцать пять находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Сериализация хранилища закрыта — двадцать пятая проверка

- **Готово:** round-trip всех 72 комбинаций перечислений через настоящий `ReviewStore`
  закреплён семью тестами. Тесты: **2381 passed**.
- **Четыре стыка проверены подряд:** `queue_builder` → `pilot_policy` (дефект, исправлен),
  `pilot_policy` → v1 (дубль, связан), `queue_builder` → `priority` (чист, закреплён),
  `models` → `storage` (чист, закреплён). Приём себя исчерпал в верстаке.
- **Что осталось без тестового покрытия в верстаке:** `issue_registry`, `reporting`.
  `storage` закрыт, `permissions` удалён, `dose_unit_audit` и `pilot_policy` прочитаны.
- **Другое направление, если продолжать:** `benchmark_review_workbench.py` и корневые
  скрипты; `clinical_engine/api/service.py` за пределами роута `/v1/guidelines/{disease_id}`.
- **Ранее закрыто за день:** двадцать четыре находки, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Стык «ось → вес» закрыт — двадцать четвёртая проверка

- **Готово:** стык между `queue_builder._regimen_axes` и `priority._WEIGHTS` закреплён
  пятью тестами. Тесты: **2374 passed**.
- **Что дал приём:** три стыка подряд оказались незакреплёнными —
  `queue_builder` → `pilot_policy` (дефект, исправлен), `pilot_policy` → v1 (дубль,
  связан), `queue_builder` → `priority` (чист, закреплён). Общий признак: словарь
  значений, который один модуль пишет, а другой читает, отбрасывая неизвестное молча.
- **Стоит проверить тем же приёмом дальше:** `TargetType` и `ReviewState` между
  `models.py`, `storage.py` и `api.py`; коды причин `reviewer_registry` против того, что
  разбирает и показывает API.
- **Что осталось без тестового покрытия в верстаке:** `issue_registry`, `reporting`,
  `storage` (16 импортов — самый используемый из непокрытых).
- **Ранее закрыто за день:** двадцать три находки, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Двадцать третья находка — дубль политики в историческом скрипте

- **Готово:** v1 больше не дублирует наборы `issue_type` и согласован с канонической
  таблицей ярусов; помечен как заменённый. Тесты: **2369 passed**.
- **Приём, который сработал второй раз подряд:** найти все места, где объявлена одна
  величина. Дубль политики в «историческом» скрипте — не архив: скрипт запускаемый, и он
  дал бы другой результат.
- **Правило, которое из этого следует:** «исторический» ≠ «неисполняемый». Если файл
  можно запустить, его расхождение с каноном — дефект, а не архивная запись.
- **Что стоит проверить тем же приёмом:** `benchmark_review_workbench.py` и другие
  корневые скрипты на дубли констант из `clinical_engine`; словари `reason_codes` между
  `priority.py` и `queue_builder.py`.
- **Что осталось без тестового покрытия в верстаке:** `issue_registry`, `reporting`,
  `storage` (16 импортов).
- **Ранее закрыто за день:** двадцать две находки, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Двадцать вторая находка — несогласованный словарь осей

- **Готово:** ярус 5 пилотного отбора теперь читает ось `missing_dose`; сквозной словарь
  осей закреплён параметризованным тестом. Тесты: **2365 passed**.
- **Приём, который сработал:** сверить словарь значений между модулем-писателем и
  модулем-читателем. Один выдаёт оси, другой их раскладывает по ярусам; расхождение
  невидимо ни в одном из них по отдельности.
- **Что стоит проверить тем же приёмом:** словари `reason_codes` и `issue_type` между
  `priority.py`, `queue_builder.py` и `service.py`; коды причин в `reviewer_registry`
  против тех, что разбирает API.
- **Ограничение среды:** базы KB (`normalized_db`, `kb_db`) в репозитории отсутствуют,
  поэтому `build_initial_queue` целиком не запускается — в нём жёсткая проверка
  численности (`regimens=1556`, `options=652`). Дефекты очереди находятся чтением и
  точечными прогонами конструкторов.
- **Что осталось без тестового покрытия в верстаке:** `issue_registry`, `reporting`,
  `storage` (16 импортов — самый используемый из непокрытых).
- **Ранее закрыто за день:** двадцать одна находка, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Двадцать первая находка — порядок отбора задавался алфавитом

- **Готово:** иерархия свидетельств в пилотном отборе сделана явной; 14 тестов.
  Тесты: **2356 passed**.
- **Приём, который сработал:** читать не только код, но и документ-первоисточник.
  `PHYSICIAN_PILOT_POLICY.md` прямо задавал иерархию, и расхождение с кодом стало видно
  только после сверки. Политика, описанная в документе, обязана сверяться с реализацией.
- **Общий признак дефекта:** порядок, зависящий от строкового сравнения имён. Если
  переименование константы меняет поведение — порядок задан не решением.
- **Что осталось без тестового покрытия в верстаке:** `queue_builder`, `issue_registry`,
  `reporting`, `storage` (16 импортов — самый используемый из непокрытых).
- **Ранее закрыто за день:** двадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Двадцатая находка — мёртвая матрица ролей

- **Готово:** `permissions.py` удалён; применяемая политика закреплена 8 тестами.
  Тесты: **2342 passed**.
- **Приём, который сработал:** спросить «а кто это вызывает?» про модуль, который
  выглядит нормативным. Матрица разрешений без единого вызова — декларация, а не
  контроль. Проверка «импортируется ли модуль хоть где-то» стоит одну команду.
- **Что осталось без тестового покрытия в верстаке:** `queue_builder`, `issue_registry`,
  `pilot_policy`, `reporting`. `dose_unit_audit` прочитан и починен, `permissions`
  удалён.
- **Ранее закрыто за день:** девятнадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Девятнадцатая находка — необъявленные имена в аннотациях

- **Готово:** два необъявленных имени в аннотациях закрыты; добавлен статический проход
  по всем 5651 аннотации репозитория. Тесты: **2334 passed**.
- **Блокирует:** push в `origin` не проходит — токен GitHub истёк
  (`The github.com token in GH_TOKEN is no longer valid`). Коммиты `c4bb044` и следующий
  лежат локально; нужен повторный вход в GitHub в Arena.
- **Неаудированные модули верстака ревью (по упоминаниям в тестах):**
  `queue_builder`, `issue_registry`, `dose_unit_audit`, `reviewer_registry`,
  `pilot_policy`, `permissions`, `reporting` — ни в одном тест-файле. Сам
  `dose_unit_audit` теперь прочитан и починен.
- **Ранее закрыто за день:** восемнадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Шестнадцатая–восемнадцатая находки — единица в точках вывода

- **Готово:** три жёстких «мг/кг» в точках вывода заменены на единицу из данных; мёртвых
  переменных в shipped-JS ноль. Тесты: **2330 passed**.
- **Приём исчерпан.** Перебор мёртвых переменных по всем 76 функциям дал одну находку и
  закрыт тестом — дальше он будет ловить только новые. Системный поиск дублирующихся
  списков тоже дал своё: расхождение `ORAL_FORMS`, падение `renderFormChips`, мёртвый
  латинский слой.
- **Что осталось непроверенным тем же духом:**
  - Python-сторона `clinical_engine/review_workbench/` (2519 строк) не аудировалась вовсе;
  - `clinical_engine/api/service.py` — только роут `/v1/guidelines/{disease_id}`;
  - цепочка сборки `db/build_db.py` → `db/regimen_semantics.py` проверялась тестами, но не
    читалась как код.
- **Ранее закрыто за день:** пятнадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Пятнадцатая находка — мёртвый латинский слой в рецепте

- **Готово:** из `fillPrescriptionForm` убраны две мёртвые переменные и две недостижимые
  карты; добавлена проверка, что локальные переменные печати не вычисляются впустую.
  Тесты: **2325 passed**.
- **Приём продолжает работать.** Системный перебор дал подряд: расхождение `ORAL_FORMS`,
  падение `renderFormChips` на трёх типах форм, мёртвые переменные в печати. Общий
  признак — объявление, которое никто не проверял на использование.
- **Что стоит проверить тем же приёмом дальше:**
  - локальные объявления в других крупных функциях (`renderPO`, `renderInjection`,
    `calculate`, `renderComboComponents`, `saveToHistory`) — тест пока покрывает только
    `fillPrescriptionForm`;
  - повторяющиеся карты `LATIN_INN` / `LATIN_FORM` против значений в БД (сейчас покрыты);
  - соответствие `AGE_GROUPS` в `db/validate_db.js`, `db/data_quality_report.py` и JS
    (сейчас совпадают: `neonate, child, adult, all`).
- **Ранее закрыто за день:** четырнадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Отрисовка форм исправлена — четырнадцатая находка

- **Готово:** `form.form_type` → `f.form_type` в `renderFormChips`; ветка `granules`
  перестала быть мёртвой. Тесты: **2324 passed**.
- **Приём, который сработал:** системный перебор списковых литералов в поиске класса
  «одно понятие в нескольких местах». Он дал три находки подряд: расхождение
  `ORAL_FORMS`, затем этот дефект. Стоит применять к другим повторяющимся структурам.
- **Что проверить тем же приёмом дальше:** повторяющиеся карты (`LATIN_FORM`,
  `LATIN_ROUTE`, `LATIN_FREQ`), списки маршрутов и возрастных групп в JS и в
  `db/validate_db.js`, а также соответствие `AGE_GROUPS` в Python и в JS.
- **Ранее закрыто за день:** тринадцать находок, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Определение пероральной формы сведено к одному

- **Готово:** `ORAL_FORMS` больше не расходится с `resolveCalculatorBinding` и основным
  правилом группировки; все три списка сверяются тестом дословно и поведением.
  Тесты: **2318 passed**.
- **Класс находок, который стоит держать в уме:** одно понятие, определённое в нескольких
  местах. Сегодня расхождение недостижимо, завтра добавят форму — и два канала начнут
  отвечать по-разному. Тест на совпадение списков дешевле, чем поиск такого дефекта.
- **Ранее закрыто:** тринадцать находок в коде, единый ворклист на все 264 находки
  валидатора, граница двух валидаторов закреплена, связь клинреков с калькулятором
  работает по обоим каналам.
- **Дальше — только решения врача или владельца:** 264 находки качества данных
  (174 заблокируют разблокировку), 82 блочные связи, 119/120 нозологий без расчёта,
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-12: Все находки валидатора стали ворклистом

- **Готово:** `db/data_quality_report.py` — единый ворклист на все девять категорий
  валидатора (264 находки, 56 нозологий). Тесты: **2317 passed**.
- **Как пользоваться:**
  ```
  python db/data_quality_report.py                                   # сводка
  python db/data_quality_report.py --json generated/data_quality_report.json
  python db/data_quality_report.py --category NO_DOSE                # одна категория
  python db/data_quality_report.py --strict                          # exit 1, если находка в открытой нозологии
  ```
- **Важное для владельца:** 174 из 264 находок станут **ошибкой сборки** после
  разблокировки нозологии (`becomes_error_when_unblocked`). Разбивка: 60 расхождений
  возрастной группы, 59 отсутствующих режимов на возраст, 27 режимов без дозы,
  27 без `regimen_label`, 1 композитная таблетка. Остальные 90 (`NO_ROUTE` 29,
  `MISSING_DURATION` 47, `UNCLASSIFIABLE_DURATION` 12, `UNROUTABLE_ROUTE` 2) останутся
  предупреждениями.
- **Что решать по каждой категории** — вопрос записан в самой строке ворклиста:
  - `NO_DOSE` (27): какую дозу назначить; рядом приведён текст КР;
  - `COMPOSITE_TABLET_BASIS` (1): доза по сумме компонентов или по первому;
  - `AGE_GROUP_MISMATCH` (60): `age_group` сценария или принадлежность режима;
  - `UNROUTABLE_ROUTE` (2): нужен ли расчёт по наружному применению;
  - `NO_REGIMEN_FOR_AGE` (59): добавить режим или убрать возраст из сценария;
  - `NO_REGIMEN_LABEL` (27): как назвать режим для закрепления в личном режиме врача;
  - `UNCLASSIFIABLE_DURATION` (12) и `MISSING_DURATION` (47): сколько дней;
  - `NO_ROUTE` (29): какой маршрут; рядом приведены доступные формы.
- **Ранее закрыто:** тринадцать находок в коде; связь клинреков с калькулятором работает
  по обоим каналам (панель HTML и `GET /v1/guidelines/{disease_id}`); граница двух
  валидаторов закреплена тестом.
- **Дальше — только решения врача или владельца:** 264 находки качества данных,
  82 блочные связи, 119/120 нозологий без расчёта, две записи с `freq_per_day: 30`
  (вероятно, артефакт извлечения «за 30-60 минут»), `netilmicin` и `tobramycin` без
  блока `dilution`.
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-11: Ворклист возрастной адресации готов

- **Готово:** `db/age_group_report.py` — ворклист по 60 расхождениям возрастной группы.
  Тесты: **2312 passed**.
- **Как пользоваться:**
  ```
  python db/age_group_report.py                                  # сводка
  python db/age_group_report.py --json generated/age_group_report.json
  python db/age_group_report.py --strict                         # exit 1 при недопустимом age_group
  ```
  Опасные направления (режим адресован старшему возрасту, чем сценарий — 19 случаев)
  стоят в начале очереди.
- **Вопрос врачу по каждой записи:** что неверно — `age_group` сценария или принадлежность
  режима этому сценарию? Калькулятор уже отказывается считать по такому сценарию, поэтому
  срочности нет, но данные надо привести в порядок до разблокировки нозологий.
- **Ранее закрыто за день:** двенадцать находок (десять в исполняемом JS, одна — связь
  проекции кроссволка с артефактом, одна — возрастная адресация), граница двух
  валидаторов закреплена тестом, оба канала связи клинреков с калькулятором проверены.
- **Остальное — решения врача или владельца:**
  - 60 расхождений возрастной группы (13 нозологий) — ворклист готов;
