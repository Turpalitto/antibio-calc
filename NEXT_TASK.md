## 2026-09-11: Аудит завершён — двенадцать находок закрыто

- **Готово:** граница двух валидаторов закреплена тестом (общее 136, только браузер 48,
  только сборка 128); рендер панели связей и восстановление из URL проверены.
  Тесты: **2301 passed**.
- **За день закрыто двенадцать находок.** Десять — в исполняемом JS калькулятора
  (единицы действия, композитная таблетка, делитель таблетки, концентрация флакона,
  длительность курса, склонение кратности, главный компонент комбинации, гранулы,
  «Скопировать назначение», фолбэк лекарственных форм), одна — связь проекции кроссволка
  с артефактом, одна — возрастная адресация сценариев.
- **Оба канала связи клинреков с калькулятором работают и проверены:** панель в HTML
  (98 из 120 нозологий, 263 связи, рендер покрыт тестами) и API
  (`GET /v1/guidelines/{disease_id}` → 200/404).
- **Дальше — только решения врача или владельца:**
  - **60 режимов в сценарии чужой возрастной группы** (39 детский/взрослый, 13 взрослый/детский,
    6 взрослый/неонатальный, 2 прочих). Расчёт по ним теперь честно не выполняется, но
    записи надо привести в порядок. Крупнейшие: разрыв ПКС 16, атопический дерматит 8,
    переломы таза 8, врождённая пневмония 6, скарлатина 4, сколиоз 4;
  - 27 режимов без числовой дозы (доза только в тексте КР);
  - 59 пар «препарат × возраст» без схемы дозирования;
  - 82 блочные связи в очереди на подтверждение;
  - 119/120 нозологий без открытого расчёта (owner-only P5.6/P6);
  - `uti_prophylaxis` / ко-тримоксазол: доза 480 мг хранится как сумма компонентов;
  - `netilmicin` и `tobramycin` имеют только `solution_iv` без `dilution`;
  - две записи с `freq_per_day: 30` — вероятно, артефакт извлечения «за 30-60 минут».
...
## 2026-09-11: Возрастная адресация закрыта — что дальше

- **Готово:** три слоя защиты от расчёта по сценарию чужой возрастной группы; браузерный
  валидатор считает то же расхождение, что и сборочный. Тесты: **2299 passed**.
- **Новый пункт для врача (данные, не код):** 60 режимов лежат в сценарии, объявленном
  для другой возрастной группы — 39 «детский во взрослом», 13 «взрослый в детском»,
  6 «взрослый в неонатальном», 2 прочих. Расчёт по ним теперь честно не выполняется, но
  сами записи нужно привести в порядок: либо исправить `age_group` сценария, либо
  перенести режим. Крупнейшие: разрыв ПКС (16), атопический дерматит (8), переломы таза (8),
  врождённая пневмония (6), скарлатина (4), сколиоз (4).
- **Ранее закрыто:** аудит исполняемого JS — десять находок; проекция кроссволка
  привязана к артефакту; оба канала связи клинреков с калькулятором проверены
  (панель HTML и `GET /v1/guidelines/{disease_id}`).
- **Остальное — решения врача или владельца, не правки кода:**
  - 27 режимов без числовой дозы (доза только в тексте КР);
  - 59 пар «препарат × возраст» без схемы дозирования;
  - 82 блочные связи в очереди на подтверждение;
  - 119/120 нозологий без открытого расчёта (owner-only P5.6/P6);
...
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-11: Проекция кроссволка закрыта — что дальше

- **Готово:** проекция в БД привязана тестами к артефакту-источнику; каждое заявленное
  число пересчитывается из данных. Тесты: **2295 passed**.
- **Проверено end-to-end:** оба канала связи клинреков с калькулятором работают —
  панель в HTML (98 из 120 нозологий со связкой, 263 связи) и API
  (`/v1/guidelines/{disease_id}` → 200/404). Превью отдаёт собранный HTML (832 844 байта).
- **Аудит исполняемого JS завершён** — десять находок закрыто. Остальное требует решений
  врача или владельца, а не правок кода:
  - 27 режимов без числовой дозы (доза только в тексте КР);
  - 59 пар «препарат × возраст» без схемы дозирования;
  - 82 блочные связи в очереди на подтверждение;
  - 119/120 нозологий без открытого расчёта (owner-only P5.6/P6);
  - `uti_prophylaxis` / ко-тримоксазол: доза 480 мг хранится как сумма компонентов;
  - `netilmicin` и `tobramycin` имеют только `solution_iv` без `dilution` — расчёт по ним
    честно не выполняется, для включения нужен блок `dilution` в справочнике;
  - две записи с `freq_per_day: 30` — вероятно, артефакт извлечения «за 30-60 минут».
- Пересборка: `python -m clinical_engine.crosswalk --write` → `python db/build_db.py` →
  `python db/build_html.py` → `python -m clinical_engine.crosswalk.review_queue --write`.
## 2026-09-11: Фолбэк форм закрыт — аудит shipped-JS завершён

- **Готово:** `ORAL_FORMS` в фолбэке группировки; пояснение при отсутствии расчётной формы.
  Тесты: **2293 passed**.
- **Аудит исполняемого JS завершён.** За день закрыто **десять находок**; список из
  `NEXT_TASK.md` вычерпан полностью (`dilution`, печатная форма, история, экспорт,
  `LATIN_FORM`, `LATIN_ROUTE`, пероральная концентрация, `component_regimens`, гранулы,
  фолбэк форм).
- **Дальше аудит имеет смысл только по данным, а не по коду.** Все оставшиеся пункты —
  решения врача или владельца, не правки:
  - 27 режимов без числовой дозы (доза только в тексте КР);
  - 59 пар «препарат × возраст» без схемы дозирования;
  - 82 блочные связи в очереди на подтверждение;
  - 119/120 нозологий без открытого расчёта (owner-only P5.6/P6).
- **Наблюдение по данным:** две записи с `freq_per_day: 30` — периоперационная
  ...
  нозологиях.
- **Наблюдение по данным:** `netilmicin` и `tobramycin` имеют только `solution_iv` без
  блока `dilution`, поэтому расчёт по ним теперь честно не выполняется. Если препараты
  нужны в расчёте, им требуется `dilution` в справочнике.
...
- **Готово:** `granules` в `isSolid`; одна форма даёт один ответ при любом весе.
  Тесты: **2287 passed**.
- **Аудит shipped-JS сошёлся.** За день закрыто восемь находок; последние пять проверок
  (`LATIN_FORM`, `LATIN_ROUTE`, пероральная концентрация, ключи `component_regimens`,
  гранулы) дефектов на текущих данных почти не дали — в основном закрепляли инварианты
  и закрывали латентные дыры.
- **Осталось из того же метода:** `copyPrescription`/экспорт как отдельная точка вывода
  (там свой набор полей, стоит сверить с печатной формой); три формы, попадающие в группу
  `po` через фолбэк-ветку группировки (`clindamycin/topical` «2% крем»,
  `tobramycin/solution_iv`, `netilmicin/solution_iv`) — проверить, корректно ли они там.
- **Наблюдение по данным:** две записи с `freq_per_day: 30` — периоперационная
  профилактика, где в тексте КР «за 30-60 минут до вмешательства». Кратность 30 раз в
  сутки выглядит как артефакт извлечения; обе без `regimen_label` и в заблокированных
  нозологиях. Стоит проверить у владельца.
