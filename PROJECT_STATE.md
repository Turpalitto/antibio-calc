## 2026-09-12: Мёртвое второе определение регулярки удалено

- **Находка 25:** `_RE_EXACT_MONTH` и `_RE_LEADING_MONTH` в `db/regimen_semantics.py`
  были определены дважды; второе перезаписывало первое. Дефект не проявлялся в данных,
  потому что оба варианта совпадают по `group(1)`, а читается только она.
- **Исправлено:** осталось одно определение, через общую константу `_MONTH_WORDS`.
  Нейтральность доказана пересборкой — SHA-256 БД не изменился.
- **Добавлено:** 3 теста, включая AST-проверку на повторные присваивания имён на уровне
  модуля — она ловит весь класс, а не только эти две регулярки.
...
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; базы KB отсутствуют в репозитории.
## 2026-09-12: Связь клинреков с калькулятором подтверждена на живых серверах

- **Оба канала работают:** калькулятор `GET /antibiotic_calc.html` → 200 (838 260 байт,
  `resolveCalculatorBinding` и `guideline_links` на месте); API
  `/v1/guidelines/aom_child` → 200 с 2 связями, `/v1/guidelines/uti_prophylaxis` → 200 с
  5 связями, несуществующий диагноз → 404 `DIAGNOSIS_NOT_FOUND`.
- **Согласованность:** 120 нозологий, 98 со связями, 263 связи; `content_sha256` в БД и
  артефакте совпадают.
- **`api/service.py` прочитан целиком, дефектов нет.** Два подозрения сняты:
  `CorpusStatus.available` — `@property`, поэтому `ready` корректно ложен при отсутствии
  корпуса (живой `/v1/health` отдаёт `degraded`); избыточная проверка «APPROVED» в v2 —
  намеренная защита от протечки словаря v1.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2409 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; базы KB отсутствуют в репозитории, поэтому
  `/v1/health` degraded и `build_initial_queue` невыполним.
## 2026-09-12: Происхождение клинических находок защищено

- **Находка 24:** `import_clinical_data_issues` писал пустой штамп `timestamp`, если в
  документе не было `meta.generated_at`. Защиты не было **нигде** в репозитории — обход
  не нашёл ни одного `raise`/`assert` по этому полю.
- **Исправлено:** отказ `ValueError` с именем файла живёт теперь в самой функции записи,
  поэтому любой путь в хранилище удовлетворяет инварианту, включая прямой вызов. Пустая и
  пробельная строки отклоняются; отказ предшествует записи; идемпотентность сохранена.
- **Штатный поток не затронут:** `clinical_data_audit.py` всегда пишет `generated_at`.
- **Добавлено:** 16 тестов в новом `tests/test_review_workbench_issue_registry.py`.
  Верстак ревью закрыт полностью: без тестового покрытия модулей не осталось.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2409 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; базы KB отсутствуют в репозитории.
## 2026-09-12: Отчётность верстака закреплена тестами

- **Проверено и чисто:** `reporting.py` — `PLANNING_MINUTES` покрывает ровно три значения
  `PriorityBand`, ключ `average_review_time_seconds` в метриках присутствует,
  `summarize_queue` отрабатывает на реальном хранилище (проекция 39 минут на трёх задачах).
- **Риск был в двух обращениях по ключу:** оба падают только в рантайме, причём первое —
  в середине агрегации, второе — после прохода по всей очереди.
- **Закреплена подпись честности:** `PLANNING_ESTIMATE_NOT_BENCHMARK` — минуты на
  приоритет являются допущением планирования, а не измерением, и рядом стоит фактическое
  среднее время ревью.
- **Добавлено:** 12 тестов в новом `tests/test_review_workbench_reporting.py`.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2393 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; `issue_registry` без тестового покрытия; базы KB
  отсутствуют в репозитории.
## 2026-09-12: Сериализация хранилища верстака закреплена

- **Проверено на настоящем `ReviewStore`:** 72 комбинации перечислений
  (`TargetType` 6 × `Severity` 4 × `PriorityBand` 3) записаны и прочитаны, 0 расхождений.
  Кортежные и вложенные поля, фильтрация, дедупликация и внешний ключ — тоже.
- **Риск был в асимметрии:** перечисления пишутся `.value`, читаются `Enum(...)`, и
  расхождение всплывает только при чтении. `TargetType` — единственное перечисление, где
  имя участника не равно значению (`CLINICAL_REGIMEN` → `"ClinicalRegimen"`).
- **Коды причин `reviewer_registry` стыка не имеют:** API передаёт их обобщённо, нигде не
  перечисляя.
- **Добавлено:** 7 тестов в новом `tests/test_review_workbench_storage_roundtrip.py`;
  `storage.py` (16 импортов) больше не самый используемый непокрытый модуль.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2381 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; `issue_registry`, `reporting` без тестового
  покрытия; базы KB отсутствуют в репозитории.
## 2026-09-12: Стык «ось → вес» закреплён

- **Проверено и чисто:** `priority.py` — все 4 значения `Severity` покрыты, все оси
  `queue_builder` и его литеральные причины имеют вес, `_HIGH_REASONS ⊆ _WEIGHTS`.
  Дублей политических констант между корневыми скриптами и `clinical_engine` не осталось.
- **Закреплено тестом:** причина без веса отбрасывается `score_priority` молча, поэтому
  новая ось без веса перестала бы влиять на порядок очереди незаметно. Теперь это падение
  теста, а не тихая потеря сигнала.
- **Четыре ключа `_WEIGHTS` без писателя** (`ambiguous_dose`, `formatting`,
  `lineage_uncertainty`, `severe_allergy`) — веса на вырост, не дефект.
- **Добавлено:** 5 тестов в `clinical_engine/review_workbench/tests/test_review_workbench.py`
  (всего 42).
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
