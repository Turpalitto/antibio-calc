## 2026-09-12: Политика пилотного отбора существует в одном месте

- **Исправлено:** `generate_pilot_review_batch.py` (v1) держал собственные копии
  `DOSE_ISSUE_TYPES`/`CONFLICT_ISSUE_TYPES` и собственную таблицу ярусов с тем же
  дефектом яруса 5, что был в `pilot_policy`. Запуск v1 дал бы другой отбор, чем
  канонический v2.
- **Решение:** v1 сохранён для воспроизводимости исторической выборки, но наборы
  `issue_type` теперь импортируются из канонического модуля (тот же объект), ярус 5
  исправлен, в шапке пометка `SUPERSEDED` с указанием v2.
- **Проверено:** словарь `DOSE_ISSUE_TYPES` соответствует фактическим кодам из отчётов
  аудита (638/116/61/367). `CONFLICT_*` — коды из проектного документа, данных с ними
  нет; ярус 6 живёт через ось `clinical_conflict`, поэтому ветка не мертва.
- **Добавлено:** 4 теста на согласие v1 с канонической политикой.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2369 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; `issue_registry`, `reporting`, `storage` без
  тестового покрытия; базы KB отсутствуют в репозитории.
## 2026-09-12: Словарь осей безопасности согласован между модулями

- **Исправлено:** ярус 5 пилотного отбора по спецификации владельца — «missing dose or
  unit», но ось `missing_dose`, которую выдаёт `queue_builder._regimen_axes`, не читалась.
  Режим без дозы при наличии единицы уходил в ярус 99 «no_owner_tier_match».
- **После правки** все семь осей маппятся в свои ярусы; единственное допустимое
  расхождение словарей — задокументированный пробел с тегом аллергии (0 из 9 153 задач),
  из-за которого ярус 4 опирается на ключевое слово.
- **Версия политики не повышена:** порядок ярусов, квоты и ключевые слова не менялись —
  исправлено соответствие уже записанной спецификации.
- **Добавлено:** 9 тестов, включая параметризованную сквозную сверку через настоящий
  `_regimen_axes`.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2365 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; `issue_registry`, `reporting`, `storage` без
  тестового покрытия; базы KB в репозитории отсутствуют, поэтому полный прогон сборки
  очереди недоступен.
## 2026-09-12: Порядок отбора в пилоте задаётся явно

- **Исправлено:** в `pilot_policy.py` порядок внутри яруса определялся строковым
  сравнением поля `origin`, из-за чего попадание по ключевому слову в свободном тексте
  обходило кураторскую сохранённую ось безопасности. Это противоречило
  `PHYSICIAN_PILOT_POLICY.md`, где производный сигнал прямо назван вспомогательным
  средством отбора, а не клиническим утверждением.
- **Правка:** явный `_ORIGIN_RANK` и ключ сортировки `_sort_key`; публичный контракт
  кортежа не изменён. Порядок больше не зависит от имён констант.
- **Добавлено:** 14 тестов на политику отбора — иерархия свидетельств, детерминизм,
  квоты, происхождение сигнала.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2356 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; `queue_builder`, `issue_registry`, `reporting`,
  `storage` без тестового покрытия.
## 2026-09-12: Политика ролей верстака закреплена тестом, а не мёртвым файлом

- **Удалено:** `clinical_engine/review_workbench/permissions.py` — матрица ролей и
  `require()`, которые не импортировались нигде в репозитории. Файл был не только
  избыточным, но и неверным: перечислял `claim_first`/`claim_second`/`administer`
  (сервис их не проверяет) и не содержал `qa_signoff`, `assign`, `view_packet`
  (сервис их проверяет). Подключение такой матрицы запретило бы `qa_signoff` всем.
- **Политика применяется двумя местами** — `service._SINGLE_ROLE_ACTIONS` и
  `reviewer_registry.validate_reviewer_action` с `CLINICAL_ACTIONS`; там, где они
  пересекаются с удалённой матрицей, расхождений нет.
- **Добавлено:** 8 тестов, закрепляющих применяемую политику, включая «администратор
  запрещён по каждому клиническому действию» — проверкой поведением, а не декларацией.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2342 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; остальные модули верстака без покрытия
  (`queue_builder`, `issue_registry`, `pilot_policy`, `reporting`).
## 2026-09-12: Все имена в аннотациях объявлены

- **Исправлено:** два случая, когда имя в аннотации не было импортировано —
  `ReviewStore` в `dose_unit_audit.import_into_store` и `ConformanceResult` в
  `bundles/loader.LoadedBundle` (второй — через `TYPE_CHECKING`, обычный импорт создал
  бы цикл). Дефект прятал PEP 563: аннотации-строки не вычисляются при загрузке.
- **Детектор статический (AST), а не `get_type_hints`:** последний роняет и корректный
  паттерн с `TYPE_CHECKING` (так ложно сработал `pipeline.StageContext`), потому что в
  рантайме блок импорта не выполняется.
- **Замер:** 351 файл, 5651 аннотация, 0 необъявленных имён.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
- **Тесты:** `pytest -q` → **2334 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта; push в origin заблокирован истёкшим токеном.
## 2026-09-12: Единица действия закрыта во всех точках вывода

- **Исправлено:** три точки вывода писали «мг/кг» жёстко — метка режима в
  `renderRegimens`, предупреждение о весе `updateWeightWarn`, строка кандидата в личном
  режиме. Все три переведены на единицу из данных (справочник или поле `unit` бандла).
- **Замеры:** 5 режимов с дозой на кг у ЕД-препарата (сообщение о весе было бы неверным);
  сочетание для метки режима сегодня не встречается (0); живых данных кандидатов в
  репозитории нет — обе последние находки латентны.
- **Мёртвых переменных в shipped-JS ноль** (было три: `freqStr`, `routeLat`, `mainRef`).
  Проверка перебирает все 76 функций верхнего уровня.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована. Превью HTTP 200.
- **Тесты:** `pytest -q` → **2330 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта.
## 2026-09-12: Из печати рецепта убран мёртвый латинский слой

- **Исправлено:** в `fillPrescriptionForm` две переменные из 29 вычислялись и не читались —
  `freqStr` (латинская кратность) и `routeLat` (латинский маршрут). Из-за этого карты
  `LATIN_FREQ` и `LATIN_ROUTE` были недостижимы, а код выглядел так, будто рецепт печатает
  латинскую кратность.
- **Проверено по форме:** латынь в рецепте принадлежит только блоку `Rp:` (название, форма,
  `D.t.d.`) и работает; кратность и маршрут по правилам идут в русскую строку `D.S.` и тоже
  работают. Латинских вариантов для них форма не предусматривает.
- **Удалено:** две мёртвые переменные, обе карты, ветка `style === 'latin'`.
- **Новое:** тест, что ни одна локальная переменная печати не вычисляется впустую —
  перебор всех объявлений. Доказан живым.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована. Превью HTTP 200.
- **Тесты:** `pytest -q` → **2325 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта.
## 2026-09-12: Выбор лекарственной формы больше не падает

- **Исправлено:** `renderFormChips` читала необъявленную переменную `form` вместо `f` в
  ветке иконки. `||` с коротким замыканием прятал дефект для `solution_im` и обнажал для
  `powder_for_injection` — того самого типа, ради которого ветка писалась. Падали также
  `granules` и `topical`.
- **Мёртвая ветка ожила:** `else if(f.form_type === 'granules')` стояла ниже падающей
  строки и была недостижима — иконка гранул не рисовалась никогда.
- **Достижимость:** `granules` попадает в группу `po` (4 вхождения в линиях),
  `powder_for_injection` — в `im` и `iv`. Выбор такого препарата ломал сетку форм.
- **После правки:** все 9 типов форм справочника отрисовываются, иконки назначаются верно.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована. Превью HTTP 200.
- **Тесты:** `pytest -q` → **2324 passed, 32 skipped, 1 xfailed** (+6 в новом
  `tests/test_calculator_form_chips.py`). При возвращённой опечатке падают 5 из 6.
- **Открыто:** 264 находки качества данных (174 заблокируют разблокировку); 82 блочные
  связи; 119/120 нозологий без расчёта.
## 2026-09-12: Одно понятие — одно определение

- **Исправлено:** «пероральная форма» определялась в трёх местах файла, и фолбэк
  `ORAL_FORMS` включал `powder_for_suspension` безусловно, тогда как личный режим врача
  и основное правило группировки требуют `concentration_mg_per_ml`. Два канала могли
  разойтись в ответе, можно ли форму принять внутрь.
- **Недостижимо на текущих данных:** все 15 форм `powder_for_suspension` имеют
  концентрацию, случаев без концентрации и без `dilution` — 0. A/B показал 0 расхождений
  между старым и новым списком, то есть это закрепление инварианта.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована. Превью HTTP 200.
- **Тесты:** `pytest -q` → **2318 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных (ворклист готов, 174 из них заблокируют
  разблокировку); 82 блочные связи; 119/120 нозологий без расчёта.
## 2026-09-12: Единый ворклист качества данных

- **Новое:** `db/data_quality_report.py` покрывает все девять категорий `validate_db.js`
  — 264 находки, 56 нозологий, по одной записи на находку с контекстом и вопросом врачу.
  Сверка с валидатором совпадает по каждой категории (264 = 264) и закреплена тестом.
- **Удалено:** `db/age_group_report.py` — стал частным случаем общего инструмента
  (`--category AGE_GROUP_MISMATCH`), а держать оба значило считать один предикат в двух
  местах. Его 11 тестов перенесены.
- **Новая информация:** 174 из 264 находок станут ошибкой сборки после разблокировки
  нозологии (поле `becomes_error_when_unblocked`). Пока все они в заблокированных
  нозологиях, поэтому сборка зелёная.
- **Исправлен дефект в самом инструменте:** 264 находки давали 250 уникальных
  `finding_id`; добавлен дискриминатор, стало 264 из 264.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. Отчёт детерминирован.
- **Тесты:** `pytest -q` → **2317 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 264 находки качества данных ждут врача и владельца; 82 блочные связи;
  119/120 нозологий без расчёта.
## 2026-09-11: Ворклист возрастной адресации

- **Новое:** `db/age_group_report.py` превращает 60 расхождений возрастной группы в
  ворклист для врача — с локализацией, дозой режима, направлением и одним вопросом.
  Опасные направления (режим старше сценария, 19 случаев) стоят первыми.
  Пишет в `generated/age_group_report.json` (в `.gitignore`), ничего не меняет.
- **Инструмент спрашивает, а не решает:** `purpose = PHYSICIAN_REVIEW_ONLY`, каждая
  запись несёт вопрос врачу и гипотезу для проверяющего.
- **Исправлен двойной учёт:** недопустимое значение `age_group` теперь учитывается только
  как нарушение политики, а не ещё и как расхождение адресации.
- **Согласованность:** отчёт на реальной БД находит ровно те же 60, что и `validate_db.js`.
  Ворклист детерминирован.
- **Тесты:** `pytest -q` → **2312 passed, 32 skipped, 1 xfailed** (+11).
- **Открыто:** 60 расхождений возрастной группы (13 нозологий) ждут решения врача;
  27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы; 82 блочные связи;
  119/120 нозологий без расчёта.
## 2026-09-11: Граница валидаторов закреплена — аудит завершён

- **Новое:** тест фиксирует разделение двух валидаторов. Общее 136 (`age_group` 60,
  `duration_days` 47, `route` 29), только браузер 48 (`нет cr_year`), только сборка 128
  (нет режима 59, нет дозы 27, нет метки 27, текст не разобран 12, маршрут 2, таблетка 1).
  Неучтённая категория с любой стороны роняет тест.
- **Проверено:** рендер панели связей клинреков уже покрыт под DOM-заглушкой;
  восстановление из URL не выбирает сценарий в обход проверки возраста.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. Превью HTTP 200, API HTTP 200.
- **Тесты:** `pytest -q` → **2301 passed, 32 skipped, 1 xfailed**.
- **Аудит завершён:** за день закрыто **двенадцать находок**. Дальнейшие пункты —
  решения врача или владельца, не правки кода.
- **Открыто:** 60 расхождений возрастной группы в данных; 27 режимов без числовой дозы;
  59 пар «препарат × возраст» без схемы; 82 блочные связи ждут врача; 119/120 нозологий
  без расчёта.
## 2026-09-11: Возрастная адресация сценариев закрыта

- **Исправлено:** сценарий, объявленный для другой возрастной группы, больше не даёт
  расчёт. В БД 60 режимов лежат в чужом сценарии; воспроизведено на `scarlet_fever` —
  взрослый режим амоксициллина (500 мг) отрисовывался внутри детского сценария
  `scarlet_std`. Сегодня недостижимо (все 60 в заблокированных нозологиях), защита
  поставлена заранее.
- **Три слоя:** `onclick` только на подходящей карточке; ворота в `calculate()`
  (в персональном режиме были, в обычном не было); та же проверка в браузерном
  `validateDB()`.
- **Шапка:** было «⚠️ 124 пред» при 264 у сборки, стало **«⚠️ 184 пред»**, из них 60 —
  по возрастной группе, как и у сборочного валидатора.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована.
  `antibiotic_calc.html` = 835 933 байта, превью HTTP 200.
- **Тесты:** `pytest -q` → **2300 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта; 60 расхождений возрастной
  группы в данных — теперь видимы в шапке и требуют решения врача.
## 2026-09-11: Проекция кроссволка привязана к артефакту

- **Новое:** два теста связывают `meta.guideline_crosswalk` в БД с артефактом
  `calculator_crosswalk.json` — по шести числам, хешу, `purpose`, `confidence_by_method`,
  и пересчётом каждого заявленного числа из данных. Раньше тесты сверяли проекцию саму
  с собой, и устаревшая проекция прошла бы незамеченной.
- **Исправлено моё утверждение:** `meta.coverage` существует — в артефакте, а не в БД.
- **Проверен API-канал связи:** `/v1/guidelines/aom_child` → 200 (2 связи, обе
  `ICD10_EXACT`, HIGH), `/v1/guidelines/pid` → 200 (6 связей), несуществующая → 404.
- **Согласованность:** все шесть общих чисел совпадают, `content_sha256` идентичен,
  фактических связей 263 = заявленному.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. БД детерминирована, кроссволк `in_sync`.
- **Тесты:** `pytest -q` → **2295 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Фолбэк группировки форм исправлен

- **Исправлено:** фолбэк в `renderForms` клал любую непопавшую форму в группу `po`.
  «2% крем» клиндамицина предлагался как пероральная суспензия с объёмом в мл
  (`concentration_mg_per_ml = 20` → `singleMg / 20`), а `solution_iv` тобрамицина и
  нетилмицина без `dilution` — как форма для приёма внутрь. Замер: было 16 случаев,
  стало 0.
- **Новое:** явный список `ORAL_FORMS`; исключено 28 проходов форм; при пустой сетке
  показывается пояснение (3 записи), а не пустой экран.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. `antibiotic_calc.html` = 832 844 байта.
- **Тесты:** `pytest -q` → **2293 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Текст копии назначения приведён к общим форматтерам

- **Исправлено:** `copyPrescription` содержала все четыре дефекта, уже закрытых в других
  точках вывода — `computeDose` без единицы, отсутствие `noDose`, сырая кратность,
  отсутствие `mainReg`. Все четыре устранены.
- **Проверка:** 611 режимов в тексте копии — 0 нулевых доз, 0 препаратов в ЕД с «мг»,
  0 неверных склонений, 0 курсов голым числом.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. `antibiotic_calc.html` = 831 265 байт.
- **Тесты:** `pytest -q` → **2291 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Ветвление пероральных форм согласовано

- **Исправлено:** гранулы в пакетике уходили в ветку «Объём не рассчитан» для ребёнка,
  а взрослому та же форма показывала обычную инструкцию по массе. `granules` добавлен
  в `isSolid`; измерено 12 изменившихся случаев «форма × вес», все — фосфомицин.
- **Замер:** ветки `renderPO` на формах группы `po` — было
  `{solid 318, adultNoConc 8, ml 102, failClosed 16}`, стало
  `{solid 330, adultNoConc 4, ml 102, failClosed 8}`.
- **Достижимость:** гранулы в открытых нозологиях — 0, все 8 в заблокированных.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. `antibiotic_calc.html` = 830 578 байт.
- **Тесты:** `pytest -q` → **2287 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Комбинации: режим главного компонента учитывается

- **Исправлено (латентно):** `calculate()` брал `reg` для главного препарата, а блок
  компонентов пропускал `refs[0]` — собственная схема главного компонента терялась бы
  целиком. Введён `mainReg`; на него переведены расчёт дозы, обе ветки отрисовки и
  метаданные. На shipped-данных 0 расхождений — изменение сохраняющее поведение.
- **Новое в валидаторе:** ключ `component_regimens` вне `combo_ref` → ERROR;
  комбинация без `component_regimens` → WARN. Обе ветки доказаны на дрейфе.
- **Замер:** 16 комбинаций, 16 режимов с `component_regimens`, 0 посторонних ключей.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. `antibiotic_calc.html` = 829 911 байт.
- **Тесты:** `pytest -q` → **2285 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Инвариант пероральной концентрации закреплён

- **Новое:** раздел 4 валидатора дополнен проверкой ``concentration_mg_per_ml`` против
  `dose_unit` препарата и положительности. Обе ветки доказаны на синтетическом дрейфе.
- **Замер:** 26 жидких форм с концентрацией, 0 у препаратов не в мг — дефекта нет,
  инвариант закреплён на будущее.
- **Проверено и чисто:** `LATIN_FORM` покрывает все 9 типов форм из БД (0 непокрытых);
  `LATIN_ROUTE` полон (`po`/`im`/`iv` — единственные возможные значения).
- **Валидатор:** EXIT=0, 0 errors / 264 warnings.
- **Тесты:** `pytest -q` → **2282 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
## 2026-09-11: Кратность приёма унифицирована

- **Исправлено:** фолбэк «N раза в день» в четырёх местах печатной формы давал
  «8 раза в день» / «12 раза в день» / «30 раза в день» — 49 режимов. Введён
  `formatFrequency(freq, style)` + `freqTimesWord(n)`; все точки вывода переведены на них.
- **Латынь не выдумывалась:** для 8/12/30 фолбэк остался русским, но грамматически верным.
- **Проверка:** склонение сверено с эталоном на 1..200 — 0 расхождений; все 8 кратностей
  из БД печатаются верно.
- **Валидатор:** EXIT=0, 0 errors / 264 warnings. `antibiotic_calc.html` = 829 181 байт.
- **Тесты:** `pytest -q` → **2279 passed, 32 skipped, 1 xfailed**.
- **Открыто:** 27 режимов без числовой дозы; 59 пар «препарат × возраст» без схемы;
  82 блочные связи ждут врача; 119/120 нозологий без расчёта.
