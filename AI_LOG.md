## 2026-09-12: Два определения одной регулярки — первое мертво

- **Находка 25.** В `db/regimen_semantics.py` (413 строки, ядро семантики данных
  калькулятора) `_RE_EXACT_MONTH` и `_RE_LEADING_MONTH` были определены **дважды**:
  строки 84 и 95 через общую константу `_MONTH_WORDS`, затем строки 162-163 —
  с явным перечислением слов. Второе определение перезаписывало первое.
- **Почему дефект не проявлялся в данных:** оба варианта совпадают по `group(1)`, а
  `parse_duration` читает только `group(1)` и `group(0)` — никогда `group(2)`. Проверено
  на семи реальных формах («3 месяца», «6 мес», «12 месяцев», «2 мес.», «3 месяца подряд»,
  «курс 3 месяца»): расхождений 0 из 7. Различие было только в числе групп — первое
  определение non-capturing (1 группа), второе capturing (2 группы).
- **Чем это опасно:** правка первого, «нерабочего» определения не дала бы ничего, и
  отладка ушла бы в неверном направлении. Тот же класс, что мёртвые локальные переменные
  `LATIN_FREQ`/`LATIN_ROUTE` в калькуляторе.
- **Правка:** вторые определения удалены, осталось одно — через `_MONTH_WORDS`, как
  недели, часы и минуты. Поведенческая нейтральность доказана пересборкой: SHA-256
  `db/antibio_db.json` **не изменился** (`51d16641760b7046…`), вывод сборки тот же
  (120 нозологий, 98 со связями).
- **Месяцы уже были закреплены** существующими тестами (`src/tests/test_regimen_semantics.py`,
  строки 51, 59, 62: FIXED 3 month, AT_LEAST 3 month, AT_MOST 6 month), поэтому единственное
  оставшееся определение покрыто.
- **Тесты:** 3 новых в том же файле (всего 60) — AST-проверка, что ни одно имя на уровне
  модуля не присваивается дважды (ловит весь класс, а не только эти две регулярки);
  что каждая из двух регулярок определена ровно один раз и строится из `_MONTH_WORDS`;
  и что месячные формы разбираются после дедупликации.
- **Проверка доказана живой:** при восстановленном дубликате падают 2 теста.
- **Своя ошибка в обвязке:** тест использовал `ast`, `Path` и путь к модулю, которых в
  файле не было; импорты и константа добавлены.
- `pytest -q` → **2412 passed, 32 skipped, 1 xfailed** (+3). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована.
...
- **Находка 24.** `issue_registry.import_clinical_data_issues` — единственный путь записи
  клинических находок в постоянное хранилище ревью. Каждая строка получает штамп
  `meta.generated_at` из документа аудита: это время **обнаружения** находки, а не время
  импорта.
- **Дефект:** при отсутствии поля штамп молча становился пустой строкой:
  ```python
  timestamp = str(document.get("meta", {}).get("generated_at") or "")
  ```
  Проверено прямым вызовом: документ без `meta.generated_at` импортировался успешно и
  давал запись с `timestamp = ''`. Находку после этого нельзя привязать к прогону аудита,
  который её нашёл.
- **Защиты не было нигде.** Обход по всему репозиторию: ни одного `raise` или `assert`,
  упоминающих `generated_at`. Моё исходное предположение, что защита живёт в
  `queue_builder`, оказалось **неверным** — метода `_add_clinical_data_issue_tasks` в
  `QueueBuilder` нет вообще, а `build_initial_queue` вызывает импорт напрямую, без
  проверки. Это тот же урок, что с `permissions.py`: контроль должен жить там, где
  инвариант, а не у одного из вызывающих.
- **Правка:** отказ `ValueError` с именем файла перенесён в саму функцию записи. Пустая
  строка и строка из пробелов тоже отклоняются. Проверено: отказ предшествует записи —
  в хранилище остаётся 0 строк; идемпотентность сохранена (повторный импорт добавляет 0).
- **Реальные производители поле пишут:** `clinical_engine/tools/clinical_data_audit.py`
  всегда заполняет `meta.generated_at`, поэтому в штатном потоке отказ не срабатывает.
  Риск — в файле, собранном вручную или другим инструментом.
- **Тесты:** 16 новых в `tests/test_review_workbench_issue_registry.py` — отказ без поля,
  отказ на пустом и пробельном штампе, имя файла в сообщении, штамп из документа,
  идемпотентность, сохранение полного payload, разрешение severity (upper, дефолт MEDIUM),
  цепочка category (`issue_code` → `problem_type` → OTHER), разрешение object_id
  (`regimen_id` → `guideline_id` → `issue_id`), поле source, placeholder для описания,
  файл с BOM, пустой документ.
- **Проверка доказана живой:** при возврате молчаливого поведения падают 3 теста.
- **Свои ошибки в обвязке:** четыре теста читали `list_issues` после выхода из `with`,
  когда соединение закрыто (`sqlite3.ProgrammingError`); чтение перенесено внутрь. Также
  в сообщении ассерта оказалось смешанноязычное «отказ должен precede записи» — исправлено.
- `pytest -q` → **2409 passed, 32 skipped, 1 xfailed** (+16). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Отчётность верстака проверена — два обращения по ключу и подпись честности

- **`reporting.py` (55 строк) не имел тестов.** Два его обращения идут по ключу и потому
  падают только в рантайме, на данных, а не при импорте:
  - `PLANNING_MINUTES[task.priority.value]` — новое значение `PriorityBand` без цифры
    планирования дало бы `KeyError` **в середине агрегации**, то есть после частичной
    работы;
  - `store.metrics()["average_review_time_seconds"]` — переименование ключа метрики дало
    бы `KeyError` **после** прохода по всей очереди.
- **Проверено и чисто:** `PLANNING_MINUTES` покрывает ровно три значения `PriorityBand`
  (HIGH 20, MEDIUM 12, LOW 7), лишних ключей нет; ключ `average_review_time_seconds` в
  `store.metrics()` присутствует (всего ключей шесть: `total`, `total_pending`, `by_state`,
  `by_type`, `by_priority`, `average_review_time_seconds`).
- **Реальный прогон `summarize_queue`** на временном хранилище: отработал без ошибок,
  проекция 20+12+7 = **39 минут**, `projected_hours` = 0.65.
- **Подпись честности закреплена отдельно.** `workload_projection.status` =
  `PLANNING_ESTIMATE_NOT_BENCHMARK`: минуты на приоритет — допущение планирования, а не
  измерение. Именно эта подпись не даёт числу быть процитированным как наблюдаемые данные,
  и рядом с ней всегда стоит фактическое `actual_average_review_time_seconds`.
- **Поведение агрегации задокументировано тестами:** ось считается по вхождениям, поэтому
  сумма `by_safety_axis` больше числа задач — это намеренно; задача без осей попадает в
  `NONE`, а не исчезает; `diagnosis` разрешается через цепочку
  `diagnosis → indication → diagnosis_id → UNKNOWN`; задача без `guideline_id` уходит в
  `UNKNOWN`.
- **Тесты:** 12 новых в `tests/test_review_workbench_reporting.py` — полнота
  `PLANNING_MINUTES` (множественное равенство, ловит и пропуск, и лишний ключ);
  монотонность минут по приоритету; наличие ключа метрик; точные срезы по severity,
  осям, диагнозам, руководствам; проекция как сумма; подпись-дисклеймер; состав
  измеренного блока; пустая очередь без делений.
- **Проверка доказана живой:** при изъятом `MEDIUM` из `PLANNING_MINUTES` — 2 падения и
  8 ошибок; ошибки как раз `KeyError` в середине агрегации, тот сценарий, ради которого
  тест писался.
- `pytest -q` → **2393 passed, 32 skipped, 1 xfailed** (+12). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Стык сериализации хранилища проверен на всех 72 комбинациях

- **Цель: `storage.py` — самый используемый из непокрытых тестами модулей верстака
  (16 импортов, 0 тест-файлов).** Рискованное место — сериализация перечислений: каждое
  пишется как `member.value` и читается обратно как `Enum(row[...])`.
- **Почему это опасно:** расхождение проявляется не при записи, а при чтении —
  `ValueError` на `TargetType(...)`. То есть дефект всплывает, когда сохранённую задачу
  загружают, возможно спустя долгое время после записи строки.
- **`TargetType` — единственное перечисление, у которого имя участника не равно значению:**
  `CLINICAL_REGIMEN` → `"ClinicalRegimen"`, и так для всех шести. Остальные
  (`Severity`, `PriorityBand`, `ReviewState`) хранятся именем. Это делает `TargetType`
  самым уязвимым: нормализация перечисления или строковый литерал в SQL сломали бы чтение.
- **Замер на настоящем `ReviewStore`:** 6 × 4 × 3 = **72 комбинации**, записаны и
  прочитаны, **0 расхождений**. Отдельно проверены кортежные и вложенные поля
  (`safety_axes`, `reason_codes`, `source_references`, `provenance_references`),
  фильтрация `list_tasks` по перечислению, отказ при повторном добавлении и внешний ключ
  на цель.
- **Внешний ключ работает и это содержательно:** задачу нельзя добавить без
  зарегистрированной цели (`IntegrityError: FOREIGN KEY constraint failed`). Без цели
  пакет ревью нечем наполнить.
- **Коды причин `reviewer_registry` сверил отдельно — стыка нет:** API передаёт их
  обобщённо через `getattr(error, "reason_code", str(error))`, нигде не перечисляя.
  Расходиться нечему.
- **Тесты:** 7 новых в `tests/test_review_workbench_storage_roundtrip.py` — все 72
  комбинации; кортежные поля; значение `TargetType` в SQL именно `"ClinicalRegimen"`;
  остальные перечисления хранятся именем; фильтрация по перечислению; повторное
  добавление отклоняется, а не дублируется; задача без цели отклоняется внешним ключом.
- **Проверка доказана живой:** при записи `.name` вместо `.value` падают 5 из 7 тестов.
- **Своя ошибка в обвязке:** первый прогон round-trip упал на всех 72 комбинациях с
  `IntegrityError` — я добавлял задачу, не зарегистрировав цель. Это свойство схемы, а не
  дефект; обвязка исправлена, а само свойство закреплено отдельным тестом.
- `pytest -q` → **2381 passed, 32 skipped, 1 xfailed** (+7). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Стык «ось → вес» был незакреплён, а причина без веса исчезает молча

- **Проверка `priority.py` дала чистый результат — и это тоже результат.** Все четыре
  значения `Severity` покрыты в `_SEVERITY_BASE` (непокрытое дало бы `KeyError` при
  обращении по ключу); все семь осей `_regimen_axes` и все литеральные причины
  `queue_builder` имеют вес в `_WEIGHTS`; `_HIGH_REASONS ⊆ _WEIGHTS`.
- **Четыре ключа `_WEIGHTS` не пишутся никем:** `ambiguous_dose`, `formatting`,
  `lineage_uncertainty`, `severe_allergy`. Это не дефект — веса на вырост и для других
  источников причин; `severe_allergy`, как и в пилотной политике, обеспечен ключевым
  словом, потому что сохранённых тегов аллергии в системе 0 из 9 153.
- **Но стык был ничем не закреплён, а он молчаливый.** `score_priority` строит вклады так:
  ```python
  contributions = [(reason, _WEIGHTS[reason]) for reason in normalized if reason in _WEIGHTS]
  ```
  Неизвестная причина **отбрасывается без ошибки и без предупреждения**. Значит, если
  `_regimen_axes` начнёт выдавать новую ось, а вес ей не добавят, ось перестанет влиять на
  порядок очереди — и это будет не видно ни в одном из двух модулей. Тот же класс, что
  `missing_dose` между `queue_builder` и `pilot_policy`.
- **Дубли констант исчерпаны:** обход по всем корневым скриптам против `clinical_engine`
  нашёл только `DB_PATH` (путь к БД, значения совпадают). Политических дублей после правки
  v1 не осталось.
- **Тесты:** 5 новых в `clinical_engine/review_workbench/tests/test_review_workbench.py`
  (всего 42) — каждая ось, которую способен выдать `_regimen_axes`, имеет вес; литеральные
  причины `queue_builder` имеют вес; у каждого значения `Severity` есть базовый балл;
  `_HIGH_REASONS` — подмножество взвешенных; неизвестная причина не падает и не меняет
  оценку (задокументированное поведение стыка).
- **Проверка доказана живой:** при изъятом весе `missing_unit` падают 2 теста.
- `pytest -q` → **2374 passed, 32 skipped, 1 xfailed** (+5). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).
...
- **Найдено при сквозной сверке двух модулей.** `queue_builder._regimen_axes` выдаёт семь
  осей безопасности: `pediatric`, `pregnancy`, `renal`, `missing_dose`, `missing_unit`,
  `clinical_conflict`, `source_mismatch`. `pilot_policy.match_tiers` читает их и раскладывает
  по ярусам.
- **Дефект:** ярус 5 по спецификации владельца в `PHYSICIAN_PILOT_POLICY.md` называется
  **«missing dose or unit»**, но в коде проверялись только `"missing_unit" in
  task.safety_axes` и `task.issue_type in DOSE_ISSUE_TYPES`. Ось `missing_dose` не читалась
  вовсе.
- **Воспроизведено:** режим без дозы, но с единицей (`dose=None, unit="мг"`) получает от
  `queue_builder` ровно одну ось — `('missing_dose',)` — и `pilot_policy` ставит его в
  ярус **99 `no_owner_tier_match`** вместо яруса 5. Контрольный случай (без единицы)
  работает верно: `('missing_unit',)` → ярус 5.
- **Правка:** добавлено `or "missing_dose" in task.safety_axes`. После неё все семь осей
  маппятся в свои ярусы: pediatric→1, pregnancy→2, renal→3, missing_dose→5,
  missing_unit→5, clinical_conflict→6, source_mismatch→7.
- **Версия политики не повышена — и это осознанно.** `PHYSICIAN_PILOT_POLICY.md` требует
  инкремента версии при изменении **порядка ярусов, квот или ключевых слов**. Здесь не
  изменилось ни то, ни другое, ни третье: порядок 1–7 тот же, квоты те же, регулярки те же.
  Исправлено соответствие реализации уже записанной спецификации яруса 5.
- **Симметричная проверка:** есть ли ось, которую `pilot_policy` читает, а `queue_builder`
  не выдаёт? Да, одна — подстрока `allerg` для яруса 4. Она **задокументирована**:
  «сохранённых тегов `renal` и `severe_allergy` в системе крайне мало или нет вовсе
  (5 и 0 из 9 153 задач)», поэтому ярус 4 опирается на ключевое слово. Расхождение
  намеренное, и теперь закреплено тестом как единственное допустимое.
- **Тесты:** 9 новых в `tests/test_pilot_selection_policy.py` (всего 23) — параметризованная
  сверка всех семи осей через настоящий `_regimen_axes` с проверкой, что происхождение
  `STORED`; регрессия на конкретный случай «дозы нет, единица есть»; и утверждение, что
  единственная нечитаемая/несохраняемая ось — задокументированный пробел с аллергией, а
  любая другая — дефект.
- **Проверка доказана живой:** при возвращённом дефекте падают 2 теста.
- **Что нельзя было проверить:** реальный прогон `build_initial_queue` — базы KB
  (`normalized_db`, `kb_db`) в репозитории отсутствуют, а сборка содержит жёсткую проверку
  численности (`regimens=1556`, `options=652`). Поэтому дефект воспроизведён на
  конструкторе осей напрямую, а не на полной очереди.
- `pytest -q` → **2365 passed, 32 skipped, 1 xfailed** (+9). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Порядок выбора в пилоте задавался алфавитом, а не силой свидетельства

- **Найдено в `pilot_policy.py`** — модуле без тестового покрытия. Ключ сортировки в
  `select_quota_capped` был кортежем `(tier, label, origin, tiebreak, task_id)`, то есть
  внутри яруса сравнение шло по строке `origin` **раньше**, чем по тай-брейку.
  Строково `"DERIVED_REVIEW_SIGNAL" < "STORED"` — просто потому, что D < S.
- **Следствие:** попадание по ключевому слову в свободном тексте **обходило** кураторскую
  сохранённую ось безопасности того же яруса. Проверено: при равном `priority_score=10`
  и `count=1` выбиралась задача `keyword-only`, а не `stored-axis`.
- **Сверка с первоисточником.** `PHYSICIAN_PILOT_POLICY.md` задаёт иерархию явно:
  `STORED` — «реальные структурированные данные», записанные `queue_builder.py` при
  приёме; `DERIVED_REVIEW_SIGNAL` — «регулярка по свободному тексту», которая
  «может дать ложное срабатывание на неотносящемся упоминании того же слова»,
  «является вспомогательным средством отбора, а не клиническим утверждением» и
  «никогда не приравнивается к суждению врача». То есть более слабое свидетельство
  выигрывало у более сильного.
- **Ключевое: порядок не был решением.** Он следовал из имён констант. Переименование
  константы молча перевернуло бы порядок обратно.
- **Правка:** явный `_ORIGIN_RANK = {"STORED": 0, "DERIVED_REVIEW_SIGNAL": 1, "NONE": 2}`
  и ключ сортировки `_sort_key`, подставляющий ранг вместо строки. Публичный контракт
  кортежа не изменён — вызывающие (`generate_pilot_review_batch_v2.py`) читают
  `origin` по индексу `[2]` и распаковывают пять элементов.
- **Проверено после правки:** сохранённая ось выбирается первой; порядок не зависит от
  порядка входа (детерминизм); внутри одного origin по-прежнему выигрывает более высокий
  `priority_score`; задача, попавшая в большее число ярусов, идёт раньше.
- **Отдельно отмечу:** в репозитории две реализации `primary_rank` — в
  `pilot_policy.py` (5-кортеж, с origin) и в `generate_pilot_review_batch.py` (4-кортеж,
  без origin). Правил только общую; вторая — исторический скрипт первой версии.
- **Тесты:** 14 новых в `tests/test_pilot_selection_policy.py` — имя политики и состав
  ярусов, сохранённая ось outrank ключевого попадания, детерминизм по порядку входа,
  явный ранг против алфавитного (с предусловием, что строково порядок обратный),
  приоритет внутри origin, число ярусов как тай-брейк, производный сигнал никогда не
  помечается сохранённым, `issue_type` sozinho достает ярусы дозы и конфликта, квоты
  (малочисленный ярус не вытесняется), верхняя граница выборки, отсутствие дублей,
  пустая очередь.
- **Своя ошибка в тесте исправлена до запуска:** я оставил битый плейсхолдер-импорт
  (`from ... import (X,) if False else None`), который не является корректным синтаксисом
  на уровне модуля. Удалён.
- **Проверка доказана живой:** при возвращённой строковой сортировке падают 2 теста.
- `pytest -q` → **2356 passed, 32 skipped, 1 xfailed** (+14). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Матрица ролей, которую никто не применял — и она же была неверна

- **Аудит верстака ревью начат с `permissions.py`** (модуль без тестового покрытия).
  Файл объявлял матрицу `PERMISSIONS` и функцию `require()` под докстрингом
  «Administrators cannot approve medicine».
- **Проверено: модуль не импортируется нигде в репозитории.** Ни сервис, ни API, ни
  `__init__.py` пакета. `require()` не вызывается ни разу. Матрица была декларацией без
  применения.
- **Политика при этом реально соблюдается — двумя другими местами:**
  1. `service._SINGLE_ROLE_ACTIONS` — действия с единственной законной ролью (защита от
     того, чтобы рецензент А вызвал `adjudicate()`, передав `role=REVIEWER_A`, роль, на
     которую он законно зарегистрирован);
  2. `reviewer_registry.validate_reviewer_action` + `CLINICAL_ACTIONS` — проверка
     личности, роли, объёма и учётных данных, включая запрет администратору выполнять
     клиническое действие (`ADMIN_CLINICAL_ACTION_FORBIDDEN`).
- **Сравнение трёх заявлений одной политики.** Там, где множества пересекаются
  (`adjudicate`, `close`, `submit_first`, `submit_second`, `waive`), они согласованы —
  противоречия нет. Но мёртвый файл **ошибочен в самом словаре действий**:
  - перечисляет `claim_first`, `claim_second`, `administer` — сервис их не проверяет
    вовсе (он использует `claim`);
  - не содержит `qa_signoff`, `assign`, `view_packet` — сервис их проверяет.
  Сервис проверяет 11 действий: `assign`, `claim`, `submit_first`, `submit_second`,
  `adjudicate`, `qa_signoff`, `request_adjudication`, `close`, `add_note`, `waive`,
  `view_packet`.
- **Решение: удалить, а не подключить.** Подключение устаревшей матрицы сломало бы
  работу: `qa_signoff` в ней нет, и он стал бы запрещён всем.
- **Ценность файла перенесена в исполняемый тест** — 8 проверок в
  `tests/test_review_workbench_role_policy.py`: администратор запрещён по каждому
  клиническому действию (поведением, через реестр); административные действия ему
  доступны; каждое проверяемое действие отнесено к клиническим или служебным; карта
  единственных ролей закреплена; каждое такое действие одновременно клиническое;
  мёртвая матрица не вернётся незамеченной; незарегистрированный не проходит ни в одной
  роли; роль обязана входить в разрешённый объём записи.
- **Проверка доказана живой:** при изъятом из `CLINICAL_ACTIONS` действии `adjudicate`
  падают 2 теста.
- **Подтверждение, что удаление безопасно:** полный набор после `git rm` — 2334 passed,
  ни одного падения, то есть от модуля ничего не зависело.
- `pytest -q` → **2342 passed, 32 skipped, 1 xfailed** (+8). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).

## 2026-09-12: Имя в аннотации, которое никто не импортировал

- **Класс тот же, что `form` вместо `f` в JS:** ссылка на имя, которого нет в области
  видимости, невидимая до момента вычисления. В Python её прячет PEP 563 —
  `from __future__ import annotations` делает аннотации строками, поэтому отсутствующий
  импорт не падает при загрузке модуля и код работает, пока что-нибудь не вычислит
  аннотацию (`typing.get_type_hints`, сериализатор, `pydantic`, проверка типов).
- **Найдено:** `clinical_engine/review_workbench/dose_unit_audit.py` —
  `import_into_store(store: ReviewStore, ...)` без импорта `ReviewStore`. Подтверждено:
  `typing.get_type_hints(...)` → `NameError: name 'ReviewStore' is not defined`.
- **Сплошной проход по `clinical_engine`** (1074 функции) дал ещё один случай:
  `clinical_engine/bundles/loader.py` — поле `conformance: ConformanceResult | None`
  без импорта. Обычный импорт там создал бы цикл: `conformance.py` сам импортирует
  `LoadedBundle` из `loader.py`, поэтому нужен `TYPE_CHECKING`.
- **Третье срабатывание оказалось ложным, и это важно.** `pipeline.StageContext`
  аннотирует `SQLiteReader`, который **уже** импортирован под `TYPE_CHECKING` — то есть
  паттерн корректный. `typing.get_type_hints` его всё равно роняет, потому что в рантайме
  `TYPE_CHECKING` ложен и блок импорта не выполняется. **Значит `get_type_hints` —
  генератор ложных срабатываний для правильного паттерна**, и детектор должен быть
  статическим (AST): объявлено ли имя в модуле вообще.
- **Правильный детектор по всему репозиторию:** 351 файл, 5651 аннотация, после правки
  **0 необъявленных имён**.
- **Две мои ошибки в проверке исправлены до фиксации:**
  - первая версия падала на файлах с BOM (`src/pipeline/extraction/__init__.py`):
    `ast.parse` не принимает U+FEFF при чтении как `utf-8` — нужно `utf-8-sig`;
  - `set(dir(__builtins__))` в **импортируемом** модуле даёт методы словаря, а не
    `int`/`str`/`list`, потому что там `__builtins__` — словарь, а не модуль. Тест
    сообщал о 4985 «необъявленных» именах, включая `int`. Исправлено на явный
    `import builtins`.
- **Тесты:** 4 новых в `tests/test_annotation_names_declared.py` — предусловие на число
  файлов, проход по всем аннотациям репозитория, и отдельная закрепка обоих найденных
  случаев (имя обязано быть именно импортировано, а не упомянуто в комментарии).
- **Проверка доказана живой:** при возвращённом дефекте падают 2 теста.
- **Среда снова сбрасывалась** посреди работы (клон отмотан на базу, `.venv` удалён,
  origin на `fce0d92`). Восстановление: `git ls-remote` за истинным tip → `git fetch` →
  `git reset <sha>` смешанный → пересоздать `.venv`. Рабочее дерево цело;
  незапушенный из-за истёкшего токена коммит перекоммичен как `c4bb044`.
- `pytest -q` → **2334 passed, 32 skipped, 1 xfailed** (+4). `node db/validate_db.js` →
  EXIT=0, 0 errors / 264 warnings. БД детерминирована (SHA-256 `51d16641760b7046…`).
