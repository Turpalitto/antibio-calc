## 2026-09-11: Расхождение двух валидаторов должно быть записано, а не замечено

- **Молчаливое расхождение обесценивает оба отчёта.** Шапка показывала «⚠️ 124 пред»,
  сборочный валидатор знал о 264. Ни одно из чисел не совпадало с другим, и ни один тест
  этого не ловил.
- **Догонять полный гейт в интерфейсе не нужно.** 312 предупреждений в строке статуса —
  шум, который пользователь перестанет читать. Правильнее разделить осознанно: браузер
  показывает то, что относится к собранному HTML, сборка — полный гейт данных.
- **Границу фиксирует тест, а не комментарий.** Любая неучтённая категория с любой стороны
  роняет тест, поэтому добавление проверки в один валидатор без другого станет явным
  решением, а не случайностью.
- **Утверждение в документации надо проверять, а не переносить.** Я записал, что ворота в
  `calculate()` переживают восстановление из URL. Причина оказалась другой: `restoreFromHash`
  вообще не выбирает сценарий. Вывод был верен, обоснование — нет, и это стоило проверки.
- **`sed` не годится для этих доков.** `**...**` ломает регулярное выражение, а правки,
  применённые до падения, выглядят как «не найдено» при повторной попытке. Только
  `edit_file` или Python, с проверкой после.

## 2026-09-11: Погашенная карточка не должна быть кликабельной

- **Визуальное предупреждение — не защита.** Карточка сценария чужого возраста гасилась
  и подписывалась «Не подходит для выбранной возрастной группы», но обработчик клика
  навешивался безусловно. Пользователь, который всё же кликнул, получал полноценный
  расчёт — взрослую дозу внутри детского сценария.
- **Отказ должен быть в двух местах.** Снятие обработчика закрывает обычный путь, но
  `calculate()` обязан проверять возрастную группу сценария сам: в персональном режиме
  такая проверка уже была, в обычном её не было вовсе. Одна защита в UI не переживает
  ни восстановления состояния из URL, ни будущего изменения разметки.
- **Два валидатора обязаны считать одно и то же.** Шапка показывала «⚠️ 124 пред»,
  сборочный валидатор знал о 264. Расхождение по возрастной группе (60) браузер не видел
  вовсе. Число в интерфейсе, которое не совпадает ни с одним отчётом, обесценивает оба.
- **Комментарий с синтаксисом вызова ломает тесты, перебирающие литералы.** Тест считал
  первое вхождение `getActiveRegimen()` определением функции; мой комментарий в другой
  функции, стоящей раньше, сместил нумерацию. Упоминать функцию в прозе можно, писать
  `имя()` — нет.
- **Разные защиты нельзя смешивать в одном утверждении.** Ворота сценария ловят
  несовпадение сценария с возрастом, подбор режима — несовпадение режима с возрастом.
  Первая версия теста смешала их и насчитала 190 «протечек» вместо 0.
- **Тип поля надо проверять, а не предполагать.** `scenario.age_group` — строка, а не
  список; `set('child')` дало набор букв и замер 638 вместо 60.

## 2026-09-11: Проекция обязана быть привязана к источнику, а не к себе

- **Тест, сверяющий производное само с собой, не ловит устаревание.** `len(links) ==
  summary["links"]` проходит и тогда, когда оба числа устарели одинаково. Проекцию в БД
  и артефакт собирают разные команды, поэтому связывать нужно именно их.
- **Заявленное число должно пересчитываться из данных.** `coverage` — не декларация:
  каждый пункт проверяется против фактического содержимого, включая тождество
  `linked + unmatched == calculator_diseases`. Иначе счётчик может разъехаться с данными
  и остаться «корректным» по форме.
- **Декларированный ноль — не расхождение.** Артефакт перечисляет все методы, включая
  `TITLE_EXACT: 0`. Сравнение обязано трактовать отсутствующий метод как ноль, иначе
  тест падает на правильной данных.
- **Проверять надо тот файл, о котором делается утверждение.** Я заявил, что ключа
  `coverage` нет, проверив `db/antibio_db.json`; ключ существует в
  `clinical_engine/resources/calculator_crosswalk.json`. Утверждение о структуре данных
  без прочтения нужного файла — не проверка.
- **Два канала одной связи проверяются оба.** Кроссволк surfaced и в панели HTML, и в
  `GET /v1/guidelines/{disease_id}`. Работа одного не означает работу другого: API
  проверен отдельно (200 с реальными связями, 404 на несуществующей нозологии).

## 2026-09-11: Фолбэк «чтобы ничего не потерять» теряет смысл назначения

- **Фолбэк в группу `po` был написан из соображения «форма не должна пропасть».** Цена
  оказалась выше: крем для наружного применения предлагался как пероральная суспензия
  с объёмом в миллилитрах, а инъекционный раствор — как форма для приёма внутрь.
  Потерять форму лучше, чем предложить её не тем путём.
- **Список допустимого явнее, чем исключение нежелаемого.** `ORAL_FORMS` перечисляет,
  что можно принять внутрь; любая новая форма по умолчанию не считается пероральной.
  Обратный вариант (чёрный список) молча пропустил бы следующую добавленную форму.
- **Пустой результат обязан быть объяснён.** Если после фильтрации форм не осталось,
  экран показывает причину и состав справочника, а не пустую сетку. Пустота выглядит
  как сбой и заставляет пользователя думать, что данные потеряны.
- **`concentration_mg_per_ml` у наружной формы — сигнал сам по себе.** У крема
  «2% крем» стоит 20 мг/мл, и этого хватило, чтобы ветка расчёта объёма приняла его
  за суспензию. Поле концентрации не определяет путь введения.

## 2026-09-11: Точка вывода, которую не проверили, содержит те же баги

- **`copyPrescription` воспроизвела все четыре закрытых дефекта.** `computeDose` без
  единицы, отсутствие `noDose`, сырая кратность, отсутствие `mainReg`. Функцию не
  проверяли вместе с остальными — и она отстала ровно на четыре правки.
- **Порядок проверок в функции вывода имеет значение.** Отказ при `noDose` обязан
  происходить до формирования текста: иначе половина документа уже собрана, и отказ
  выглядит как обрыв, а не как решение. Тест проверяет именно порядок
  (`body.index("if(noDose)") < body.index("let txt =")`).
- **Копия в буфер обмена — такой же документ, как печатная форма.** «0 мг» в буфере
  неотличимо от реальной нулевой дозы, и текст уйдёт во внешнюю систему (историю
  болезни, мессенджер), где его никто не перепроверит.
- **Мёртвая ассерция хуже отсутствующей.** `assert ... or True` всегда проходит и создаёт
  видимость покрытия. Такая строка была найдена и заменена на содержательную проверку
  до того, как тест был запущен.
- **Список точек вывода стоит вести явно.** Пять мест печатают дозу: экран (per os),
  экран (инъекция), печатная форма, копия в буфер, история. Каждый новый форматтер нужно
  применять ко всем пяти сразу.

## 2026-09-11: Мерять надо ту ветку, которая действительно исполняется

- **Первый замер был завышен в 7,5 раза.** Я перечислил все формы справочника и получил
  120 «fail-closed», но `renderPO` вызывается только для группы `po`. Инъекционные флаконы
  туда не попадают. Без копии логики группировки из `renderForms` число было бессмысленным.
  Правильный замер дал 16, а после правки — 8.
- **Список «твёрдых» форм — это утверждение о способе дозирования.** Пакетик гранул
  дозируется по массе; миллилитры пакетика не измеряют. Отсутствие `granules` в списке
  означало, что форма попадает в ветку для жидкостей с неизвестной концентрацией.
- **Одна форма не должна давать разный ответ в зависимости от веса.** Порог ``w >= 40``
  законно разделяет «взрослому можно по массе» и «ребёнку нужен объём» для жидкостей,
  но для формы, которая в принципе не дозируется по объёму, он создавал ложное
  предупреждение.
- **Латентный дефект с измеренной недостижимостью всё равно чинится.** Гранулы в открытых
  нозологиях — 0 случаев. Но дефект молчаливый, а данные меняются; цена правки — одна
  строка и тест.

## 2026-09-11: Пропуск элемента в цикле — это тоже решение о данных

- **`refs.slice(1)` означало «главный компонент обработан где-то ещё».** Но основной расчёт
  брал `reg` напрямую, поэтому собственная схема главного препарата не обрабатывалась
  нигде. Пропуск в одном месте и умолчание в другом вместе дали дыру, которой не видно
  ни в одном из них по отдельности.
- **Один режим — один источник для всех полей вывода.** `mainReg` передан в `computeDose`,
  `renderPO`, `renderInjection` и `renderDrugMeta`. Иначе доза считалась бы от одного
  режима, а длительность и кратность печатались от другого.
- **Посторонний ключ в словаре по drug_ref — ошибка, а не лишний ключ.** Ключ вне
  `combo_ref` называет препарат, которого в записи нет, и молча игнорируется всеми путями
  отрисовки. Такой ключ невозможно заметить в интерфейсе.
- **Отсутствие `component_regimens` у комбинации — предупреждение.** Все компоненты
  наследуют режим линии; это законно, но должно быть видно, иначе комбинация из двух
  препаратов с разными дозами выглядела бы как одна схема.
- **Сохраняющее поведение доказывается замером, а не рассуждением.** 0 режимов, где
  `mainReg` отличается от `reg`, — вот почему правка безопасна на текущих данных.

## 2026-09-11: Инвариант закрепляется и когда дефекта нет

- **Пероральная ветка делит на ``concentration_mg_per_ml`` безусловно.** Для препарата в
  ЕД это смешало бы единицы. На shipped-данных сочетания нет (26 жидких форм, 0 у
  препаратов не в мг), но одна новая форма у бензилпенициллина создала бы его молча.
  Проверка в валидаторе стоит дешевле, чем разбор рецепта с неверным объёмом.
- **Полнота словаря измеряется, а не предполагается.** ``LATIN_FORM`` оказался полным
  (9 из 9 типов форм), ``LATIN_ROUTE`` — тоже (единственные возможные значения
  `po`/`im`/`iv`). Фолбэки ``|| form.form_type`` и ``|| ''`` на текущих данных
  недостижимы; утверждать это можно только после замера.
- **Лишний ключ в словаре — не дефект.** `syrup` и `suppository` в `LATIN_FORM` есть,
  а в БД таких форм нет. Это запас на будущее расширение справочника.
- **Аудит имеет конец.** Когда три проверки подряд не дают находок, класс исчерпан и
  пора менять метод, а не продолжать перебирать экраны.

## 2026-09-11: Словарь с фолбэком требует проверенного фолбэка

- **Фолбэк — это тоже вывод, и он обязан быть корректным.** `LATIN_FREQ` покрывает
  {1,2,3,4,6}; кратности 8, 12 и 30 падали в «N раза в день». Словарь был курируемым,
  а фолбэк — нет, и именно фолбэк печатался в 49 рецептах.
- **Склонение выносится в функцию и сверяется с эталоном на диапазоне.** `freqTimesWord`
  проверена на 1..200 против независимой реализации правила. Точечные тесты на 8 и 12
  не поймали бы ошибку, скажем, на 11–14.
- **Не выдумывать термины, которых нет в источнике.** Для 8/12/30 латинских названий
  в `LATIN_FREQ` нет; «octies in die» пришлось бы сочинить. Фолбэк оставлен русским, но
  грамматически верным — честнее, чем правдоподобная выдуманная латынь в рецепте.
- **Заплатка поверх готового форматтера — признак того, что форматтер применён не там.**
  `.replace(' в день в день',' в день')` возник потому, что я подставил `formatFrequency`
  в строку, где «в день» уже было. Правильный ход — убрать дубль, а не глушить его.
- **Отсутствие значения — прочерк, а не `null` в тексте.** `formatFrequency(null)` → «—»;
  иначе рецепт напечатал бы «null раза в день».

## 2026-09-11: Форматтер обязателен в каждой точке вывода, а не только в «главной»

- **Сырое поле в панели — это не «проще», это другой ответ.** `duration_days` хранит
  «10-14», «5», «1»; `formatDuration` превращает это в «10–14 дней», «5 дней»,
  «однократно». 459 из 638 режимов выглядели в панелях не так, как в блоке комбинации.
  Терялась и семантика: «однократно» и «пожизненно» становились числами.
- **Проверять подпись поля перед заменой на форматтер.** Подписи «Курс» не содержат
  единицы, поэтому «10–14 дней (дней)» не возникает. Если бы подпись была «Курс, дней»,
  правка породила бы новый баг.
- **История — запись расчёта, а не черновик.** Сохранять «0 мг» для режима без числовой
  дозы нельзя: в списке это неотличимо от реальной нулевой дозы. Отказ с сообщением
  согласован с тем, что делает экран.
- **Единица препарата передаётся явно везде.** `saveToHistory` вызывал `computeDose` без
  третьего аргумента. Математика от него не зависит, но запись истории хранит `unit` —
  и брать её надо из того же места, что и остальные точки вывода.
- **Поле в истории версиируется с откатом.** Записи до появления `unit` показываются
  в «мг», а не ломают отрисовку на `undefined`.

## 2026-09-11: Рефакторинг доказывается сверкой со старой реализацией

- **`||`-фолбэк по двум полям — это выбор, записанный трижды.**
  ``final_concentration_mg_ml || final_concentration_units_ml`` встречался в подсказке
  «на дозу», в блоке разведения и в латинской форме рецепта. Хелпер выбирает поле
  явными проверками ``> 0``, поэтому фолбэка в шаблоне не остаётся вовсе — и разойтись
  больше нечему.
- **Мёртвая переменная — признак незавершённого намерения.** `isUnits` в
  `computeInjectableMl` считалась и не использовалась: автор собирался различать единицы,
  но не сделал этого. Такие места стоит читать как список того, что не доделано.
- **Сокращение «тыс»/«млн» применимо только к единицам действия.** `formatUnits(1000)`
  даёт «1 тыс», а в БД есть `vial_mg` 1000/1500/2000/4000 — метка флакона стала бы
  «1 тыс мг». Регрессия была найдена замером значений до фиксации, а не после.
- **Консолидация без сверки — это переписывание наугад.** Старые три реализации
  воспроизведены в тесте дословно и сравнены с новыми на всех 162 парах
  «флакон × режим». Ноль расхождений — единственное доказательство, что поведение
  сохранено, а не изменено вместе с дублированием.
- **Обёртка сохраняет сигнатуру.** `computeInjectableMl` оставлен тонкой обёрткой над
  `injectableMlForDose`, чтобы вызывающие не менялись: меньше мест, где можно ошибиться.

## 2026-09-11: Одна величина — одно извлечение

- **Регулярка «последнее число перед единицей» берёт не тот компонент.** ``/(\d+)\s*мг/``
  на «875+125 мг» возвращает 125. Для комбинации это всегда второй компонент — то есть
  наименее значимый. Делитель таблетки извлекается через `parseFloat`, который
  останавливается на «+» и даёт основной компонент.
- **Подсказка на экране и печатная форма обязаны считать одинаково.** Они расходились в
  7 раз на одной и той же дозе: «7 таб 125 мг» против «1 таб». Обе точки переведены на
  `tabletStrengthMg(form)`; grep-тест закрепляет, что `parseFloat(form.concentration)`
  встречается ровно один раз — внутри самого хелпера.
- **Все три бага дня — один класс.** Единицы действия («мг» вместо «ЕД»), базис дозы
  (сумма против первого компонента), делитель таблетки (последнее число против первого) —
  во всех случаях одна величина извлекалась независимо в нескольких местах. Аудит
  эффективнее начинать с поиска дублирующих извлечений.
- **Отсутствующая прочность — не ноль и не NaN на экране.** Если делитель не извлекается,
  signa не печатается вовсе, а подсказка о таблетках возвращается пустой строкой.

## 2026-09-11: Расхождение базисов ловится точным правилом, а не порогом

- **`parseFloat` останавливается на «+».** Форма «400+80 мг» даёт делитель 400. Если доза
  в данных — сумма (480), результат 1.2 таблетки: физически невозможное число в рецепте.
  Оба числа настоящие, поэтому ни проверка диапазона, ни проверка положительности его
  не видят.
- **Шумный порог хуже отсутствия проверки.** Правило «число таблеток не кратно 1/4»
  дало 749 срабатываний из 2324 (32%), из которых почти все — законные педиатрические
  дозы мг/кг. Такой сигнал никто не станет читать, и настоящий случай в нём потеряется.
  Точное правило — доза кратна сумме компонентов, но не кратна первому — срабатывает
  ровно один раз и указывает прямо на дефект.
- **Дробная таблетка сама по себе не ошибка.** `renderPO` уже печатает «~1/2 таб»;
  половина таблетки 500 мг — реальная практика. Ошибка — только когда число таблеток
  не выражает никакую долю, которую можно отмерить.
- **Проверка обязана срабатывать на испорченных данных.** Эскалация WARN → ERROR при
  снятии `calculation_blocked` доказана тестом: иначе невозможно отличить работающую
  проверку от отсутствующей.

## 2026-09-11: Инвариант закрепляется, даже когда данные пока корректны

- **Аудит без находки — не повод не закреплять правило.** `dilution` оказался чист
  (0 расхождений на 81 опции), но калькулятор делит дозу на то поле концентрации,
  которое присутствует. Один неверный флакон у препарата в ЕД дал бы молча неверный
  объём. Проверка стоит дёшево, а цена ошибки — та же, что у уже найденных багов.
- **Оба поля концентрации сразу — ошибка, а не выбор.** Если у флакона указаны и
  `final_concentration_mg_ml`, и `final_concentration_units_ml`, калькулятор не может
  решить, на что делить. Молчаливый приоритет одного поля скрыл бы расхождение.
- **Инструмент проверки должен быть проверяем.** `validate_db.js` читал путь жёстко,
  поэтому первую попытку подтвердить живость новой проверки валидатор попросту не
  увидел — и «подтвердил» ноль ошибок на файле, который не открывал. Путь стал
  аргументом; каждая ветка теперь доказана на намеренно испорченных данных.
- **Дрейф-тест обязателен для каждой новой проверки.** Проверка, которая ни разу не
  сработала на испорченных данных, неотличима от отсутствующей.

## 2026-09-11: Единица дозы — свойство препарата, а не константа в коде

- **Имя поля не определяет единицу.** `single_dose_mg` и `dose_mg_day_fixed` для
  бензилпенициллина хранят единицы действия. Жёсткое «мг» в подписи давало
  «4000000 мг 6 р/д» — четыре килограмма пенициллина. Единица берётся из
  `drugs_reference[drug_ref].dose_unit` и на сборке, и в UI.
- **Рукописные метки были индикатором.** Они уже говорили «2-4 млн ЕД 6 р/д», пока
  сгенерированные рядом говорили «мг». Расхождение между проверенным человеком и
  выведенным машиной — первый сигнал, что вывод неверен.
- **Одна функция форматирования на все места вывода.** Signa рецепта, текст для
  копирования и история приписывали «мг» каждая по-своему. Все переведены на
  `fmtDose()`; расхождение между точками вывода — тот же источник ошибок, что
  расхождение между ветками `computeDose()`.
- **Поле в истории версиируется с откатом.** Записи, сохранённые до появления `unit`,
  показываются в «мг», а не ломают отрисовку на `undefined`.
- **Отсутствие дозы — это состояние, а не ноль.** Режим без дозовых полей давал
  `singleMg = 0` при `needWeight = false`, и экран печатал «0 мг» как дозу. Теперь
  `computeDose()` возвращает явный `noDose`, а UI объясняет, что числовой дозы в КР нет.
  Ноль выглядит как доза — это и есть опасность.
- **Доза из свободного текста не выводится.** У 27 режимов значение есть только в
  `duration_note` («в первый день по 1 млн ЕД/кг/сут…»). Парсить его в число означало бы
  выдумать дозу; калькулятор отказывается считать и указывает на примечание (INV-09).
