## 2026-09-12: Повторное присваивание — дефект, который не видно в данных

- **Два присваивания одному имени на уровне модуля — первое мертво.** Python не
  предупреждает: второе просто побеждает. Если оба значения совпадают по тому, что
  читает код, дефект не проявляется ни в одном тесте и ни в одной сборке.
- **Проявится он в день отладки.** Кто-нибудь поправит первое определение, не увидит
  эффекта и начнёт искать причину не там. Стоимость дефекта — не неверные данные, а
  потерянное время.
- **Проверка должна ловить класс, а не экземпляр.** AST-обход по присваиваниям на уровне
  модуля находит любое повторное имя в файле, а не только две известные регулярки.
- **Нейтральность правки доказывается пересборкой, а не рассуждением.** Идентичный
  SHA-256 `db/antibio_db.json` — это измерение, а не утверждение.
- **Общая константа лучше явного перечисления.** `_MONTH_WORDS` используется и для недель,
  часов и минут; дубликат с явным списком слов расходился с ней по числу групп, хотя
  совпадал по смыслу.

## 2026-09-12: Свойство или метод — это различие, которое видно только на живом ответе

- **`ready = st.available` читается одинаково для свойства и метода.** Если бы
  `available` был методом, переменная получила бы связанный метод — всегда истинный —
  и health сообщал бы «готов» при полностью отсутствующем корпусе. Различие видно в
  исходнике (наличие `@property`) и подтверждается живым ответом: `ready=False`.
- **Избыточное условие может быть намеренным.** `status == "APPROVED" or status not in
  ALLOWED_STATUSES` — первая часть никогда не срабатывает, но фиксирует намерение:
  словарь одобрения v1 не должен появиться в v2. Удалять такое «упрощение» стоит только
  вместе с пониманием, зачем оно там.
- **Проверка на живом сервере ловит то, что тесты не покрывают.** Тесты проверяют
  обработчики на подставных зависимостях; фактический HTTP-ответ показывает, что адаптер
  FastAPI, сериализация и маршрутизация тоже согласованы.
- **Degraded — это корректный ответ, а не ошибка.** Отсутствие баз KB даёт
  `status=degraded, ready=False` при HTTP 200; 503 зарезервирован для отказа
  рекомендовать. Смешивать эти два сигнала нельзя.

## 2026-09-12: Инвариант без контроля — это отсутствие контроля, даже если данные пока правильные

- **`or ""` превращает отсутствующее обязательное поле в валидное значение.** Пустая
  строка проходит в базу, и дефект становится невидимым: запрос работает, поле есть,
  содержание потеряно. Обязательное поле должно отказывать, а не подставляться.
- **Контроль belongs там, где пишется строка.** Защита у одного вызывающего не защищает
  прямой вызов; а если вызывающий один — защита исчезает вместе с его рефакторингом.
  В этом репозитории защита не существовала вовсе, хотя инвариант был сформулирован
  в назначении поля.
- **Штамп из документа, а не из времени импорта — содержательное различие.** Время импорта
  говорит, когда данные загрузили; `meta.generated_at` говорит, когда находку обнаружили.
  Подмена одного другим молча меняет смысл поля.
- **Отказ должен предшествовать записи.** Проверка после цикла импорта оставила бы
  частично заполненное хранилище. Тест на «0 строк после отказа» фиксирует именно это.
- **Проверять штатный поток и дефектный путь надо раздельно.** Реальный производитель
  поле пишет, поэтому в штатном потоке ничего не ломается — и именно поэтому дефект
  не был заметен.

## 2026-09-12: Обращение по ключу — это контракт на полноту, а не на удобство

- **`PLANNING_MINUTES[task.priority.value]` падает не при импорте, а на данных.** Хуже
  того — в середине агрегации: часть работы уже сделана, результат потерян. Проверка
  полноты словаря против перечисления стоит одну строку теста.
- **Множественное равенство ловит обе ошибки.** `set(PLANNING_MINUTES) == {band.value
  for band in PriorityBand}` падает и на пропущенном значении, и на лишнем ключе.
  Проверка только «все покрыты» пропустила бы устаревший ключ.
- **Подпись-дисклеймер — часть контракта, а не комментарий.**
  `PLANNING_ESTIMATE_NOT_BENCHMARK` — единственное, что мешает 39 минутам быть
  процитированными как измерение. Такое поле надо закреплять тестом наравне с числом.
- **Сумма больше числа элементов — не всегда ошибка.** `by_safety_axis` считает ось по
  вхождениям, и задача с двумя осями попадает в оба среза. Тест фиксирует это как
  намеренное, иначе следующий аудитор примет за дефект.
- **Пустой вход — обязательный случай.** Пустая очередь проверяет, что в агрегации нет
  деления на число задач и что срезы возвращаются пустыми, а не отсутствующими.

## 2026-09-12: Асимметричная сериализация откладывает ошибку до чтения

- **Запись `.value`, чтение `Enum(...)` — контракт, который никто не проверяет.** Он
  выполняется, пока значения перечисления совпадают с тем, что лежит в базе. Расхождение
  не ломает запись: оно ломает следующее чтение, возможно через месяцы.
- **Перечисление, у которого имя не равно значению, — точка риска.** `TargetType` хранит
  `"ClinicalRegimen"`, а не `"CLINICAL_REGIMEN"`. Любой, кто «приведёт к единому стилю»
  или впишет значение строкой в SQL, сломает чтение всех существующих строк. Тест на
  конкретное значение в SQL делает такую правку видимой.
- **Ошибка в моей обвязке оказалась свойством схемы.** `IntegrityError` на всех 72
  комбинациях выглядел как провал проверки, а был внешним ключом: цель регистрируется
  раньше задачи. Свойство стоит закрепить отдельным тестом, а не обходить молча.
- **Не у каждого словаря есть стык.** Коды причин `reviewer_registry` API передаёт
  обобщённо, нигде не перечисляя, — расходиться нечему. Проверить это дешевле, чем
  писать тест на отсутствие проблемы.
- **Проверка на всех комбинациях, а не на одной.** 72 комбинации стоят столько же, сколько
  одна, и покрывают каждого участника каждого перечисления. Точечный тест пропустил бы
  участника, добавленного позже.

## 2026-09-12: Молчаливое отбрасывание — самое дорогое место для расхождения

- **`if reason in _WEIGHTS` без `else` — это тихая потеря сигнала.** Новая ось без веса не
  упадёт, не предупредит и не изменит поведение заметно: задача просто получит меньший
  балл. Обнаружить это можно только сверкой двух модулей.
- **Чистый результат проверки тоже надо закреплять.** `priority.py` оказался корректным, но
  ничто не мешало ему перестать им быть при следующем добавлении оси. Тест на стык дешевле,
  чем повторный аудит.
- **Отсутствие писателя у ключа — не всегда дефект.** Четыре веса в `_WEIGHTS` никто не
  пишет; три из них — запас на будущие источники, один обеспечен ключевым словом по
  задокументированной причине. Удалять их значило бы сломать будущие источники.
- **Обращение по ключу словаря — это контракт на полноту перечисления.**
  `_SEVERITY_BASE[severity]` даст `KeyError` на новом значении `Severity`. Такой словарь
  обязан сверяться с перечислением тестом, а не надеждой.
- **Три стыка подряд — это не совпадение, а свойство архитектуры.** Словари значений,
  которые пишет один модуль и читает другой, встречаются в верстаке постоянно; проверять
  их надо по списку стыков, а не по вдохновению.

## 2026-09-12: «Исторический» скрипт, который можно запустить, — не архив

- **v1 не мёртв — он исполняем.** Он лежит в корне, у него `main`, и он дал бы выборку,
  отличающуюся от канонической. Держать в таком файле копию политики — значит держать
  второй источник истины, который уже разошёлся.
- **Связать дешевле и безопаснее, чем удалить.** Удаление лишило бы воспроизводимости
  исторической выборки, на которую ссылаются отчёты. Импорт канонических наборов убирает
  возможность расхождения, а пометка `SUPERSEDED` убирает возможность принять v1 за
  актуальный инструмент.
- **Проверка через `is`, а не через равенство.** `v1.DOSE_ISSUE_TYPES is DOSE_ISSUE_TYPES`
  утверждает, что это один объект, а не два совпадающих набора. Равенство прошло бы и
  после возвращения копии.
- **Не каждый словарь-призрак — дефект.** `CONFLICT_*` не встречается в данных, но
  описан в проектном документе как будущий флаг, а ярус 6 достижим через сохранённую ось.
  Такую ветку надо закрепить тестом как намеренную, иначе следующая проверка объявит её
  мёртвым кодом и удалит.
- **Перенос приёма без переноса объявлений.** Я использовал `ROOT` в файле, где его не
  объявил, потому что в других тест-файлах он есть. Приём копируется целиком или не
  копируется вовсе.

## 2026-09-12: Словарь значений живёт между модулями, и проверять надо стык

- **Ни один из двух модулей не был ошибочным сам по себе.** `queue_builder` честно выдавал
  ось `missing_dose`; `pilot_policy` честно читал `missing_unit`. Дефект существовал только
  на стыке и был невидим при чтении любого из них по отдельности.
- **Название яруса — часть спецификации.** Ярус называется «missing dose or unit», и это
  запись в документе владельца. Если реализация проверяет только половину названия, это
  расхождение с первоисточником, а не вопрос вкуса.
- **Не каждое расхождение словарей — дефект.** Ось аллергии читается, но не сохраняется —
  и это прямо задокументировано («0 из 9 153 задач»). Такой случай надо закреплять тестом
  как исключение, иначе следующая проверка объявит его ошибкой.
- **Версию политики повышают за изменение политики.** Документ требует `PHYSICIAN_PILOT_V2`
  при смене порядка ярусов, квот или ключевых слов. Соответствие уже записанной
  спецификации — исправление, а не новая версия; повысить версию здесь значило бы
  замаскировать исправление под изменение решения.
- **Отсутствие данных — не повод пропускать проверку.** Баз KB в репозитории нет, полный
  прогон сборки очереди недоступен. Конструктор осей вызывается напрямую, и этого
  достаточно, чтобы воспроизвести дефект и закрепить его тестом.

## 2026-09-12: Если переименование константы меняет поведение, порядок не был решением

- **Сортировка по строковому полю — скрытая политика.** Кортеж
  `(tier, label, origin, tiebreak, task_id)` сравнивал `origin` раньше тай-брейка, и
  «DERIVED_REVIEW_SIGNAL» < «STORED» оказалось содержательным утверждением о том, чьё
  свидетельство сильнее. Никто этого не утверждал — так сложились буквы.
- **Документ-первоисточник разрешает спор.** `PHYSICIAN_PILOT_POLICY.md` описывает
  производный сигнал как регулярку, способную дать ложное срабатывание, и запрещает
  приравнивать его к суждению врача. Без сверки с документом я мог бы решить, что
  порядок намеренный, и «закрепить» его тестом.
- **Явный ранг дешевле, чем правильные имена.** `_ORIGIN_RANK` с числами делает намерение
  видимым и не зависит от того, как названы константы. Тест дополнительно утверждает
  обратное алфавиту — чтобы случайное совпадение имён не прошло как корректность.
- **Контракт кортежа трогать не надо.** Вызывающие читают `origin` по индексу и
  распаковывают пять элементов; подстановка ранга в ключ сортировки меняет поведение, не
  меняя форму данных. Это меньшая правка, чем добавление поля в кортеж.
- **Плейсхолдер-импорт — не черновик, а синтаксическая ошибка.** Строка
  `from m import (X,) if False else None` невалидна на уровне модуля; она бы упала при
  сборе тестов, а не при проверке.

## 2026-09-12: Нормативный файл без вызова — это отсутствие контроля, а не его наличие

- **Матрица разрешений, которую никто не импортирует, не защищает ничего.** Она читается
  как источник истины о том, кто что может, и именно поэтому вводит в заблуждение:
  аудит or новый разработчик сверится с ней и сделает неверный вывод.
- **Перед удалением надо проверить, согласована ли она с тем, что применяется.** Здесь
  пересечения совпали — значит, политика не расходилась, расходился словарь действий.
  Это другое: устаревший файл, а не противоречивый.
- **Подключать устаревшую матрицу было бы хуже, чем удалить её.** В ней нет `qa_signoff`,
  поэтому подключение запретило бы это действие всем. «Приведём код в соответствие с
  декларацией» — опасный рефлекс, если декларация неверна.
- **Ценность декларации переносится в тест, а не в комментарий.** «Администратор не
  утверждает лекарства» теперь проверяется поведением реестра по каждому клиническому
  действию. Удалить такой тест заметно; удалить строку в мёртвом файле — нет.
- **«Кто это вызывает?» — дешёвый вопрос с высоким выходом.** Одна команда grep по
  импортам модуля показала, что нормативный слой отключён. Тот же вопрос стоит задать
  каждому модулю, который выглядит как политика или контракт.

## 2026-09-12: Отложенная аннотация откладывает и ошибку в ней

- **PEP 563 делает аннотацию строкой, а не проверкой.** `store: ReviewStore` без импорта
  не падает ни при загрузке, ни при вызове — только когда что-нибудь вычислит аннотацию.
  В проекте с `pydantic` и FastAPI это «что-нибудь» обязательно появится.
- **Циклический импорт — не повод писать аннотацию без имени.** `ConformanceResult`
  нельзя импортировать обычно, потому что `conformance.py` импортирует `LoadedBundle`
  обратно. Решение — `TYPE_CHECKING`, а не отсутствие импорта.
- **Детектор должен отличать «не импортировано» от «импортировано только для типов».**
  `typing.get_type_hints` не отличает: в рантайме `TYPE_CHECKING` ложен, блок не
  выполняется, и корректный код падает так же, как дефектный. Мой первый проход на нём
  дал три срабатывания, одно из которых — ложное. Статический обход AST даёт ровно два.
- **`__builtins__` в импортируемом модуле — словарь.** `set(dir(__builtins__))` вернул
  методы словаря вместо `int`/`str`/`list`, и тест заявил о 4985 «необъявленных» именах.
  В скрипте `python -c` то же выражение работает — контекст меняет тип объекта.
- **`ast.parse` не терпит BOM.** `src/pipeline/extraction/__init__.py` начинается с
  U+FEFF; чтение нужно через `utf-8-sig`. Это не дефект файла, а требование обхода.

## 2026-09-12: Точка вывода, которую не проверили, хранит каждый закрытый дефект

- **Находка №1 была закрыта в четырёх местах и осталась в трёх других.** Жёсткая единица
  в точке вывода — не один дефект, а класс: каждая строка, печатающая «мг», независима.
  Закрывать надо перебором всех строк, где единица написана литералом, а не по списку
  известных мест.
- **Мёртвая переменная указывает на соседа.** `mainRef` в `renderRegimens` сам по себе
  безвреден, но строка рядом печатала «мг/кг» жёстко. Проверяя одно, стоит читать
  окружающий код, а не только найденное.
- **Обязательное поле схемы, которое отображение игнорирует, — это дефект.** В бандле
  личного режима `dose.unit` обязательно, а строка писала «мг/кг/сут» литералом. Данные
  уже несут правильный ответ; вопрос только в том, читает ли его код.
- **Латентный дефект всё равно стоит закрыть, если цена — одна строка.** Ни одна из трёх
  находок не воспроизводится на текущих данных. Но все три — тот же класс, что уже
  проявлялся, и все три закрываются заменой литерала на общий определитель.
- **Фикстура без обязательного поля проверяет фолбэк, а не поведение.** Мой `activeDrug`
  без `drug_ref` заставлял единицу уходить в «мг», и тест падал на неполной фикстуре.
  Фикстура обязана содержать то, что требуется реальному объекту.
- **Заглушка, создающая потомка в конструкторе, рекурсивна.** `RangeError: Maximum call
  stack size exceeded` — не дефект кода, а моя обвязка. Потомок создаётся лениво, по
  первому запросу.

## 2026-09-12: Переменная, которую никто не читает, — это неправда о программе

- **Два вычисления из двадцати девяти ничего не значили.** `freqStr` и `routeLat`
  выглядели как часть рецепта, но никуда не подставлялись. Читатель кода заключал, что
  рецепт печатает латинскую кратность. Он не печатал.
- **Недостижимая курируемая карта хуже её отсутствия.** `LATIN_FREQ` и `LATIN_ROUTE`
  содержали правильный медицинский латинский, но исполниться не могли. Пока они лежали
  рядом с кодом, казалось, что фича есть.
- **Сверка с первоисточником решает, удалять или подключать.** В российском рецепте
  латынь принадлежит блоку `Rp:`, а кратность и маршрут — русской строке `D.S.`. Значит
  подключать было некуда, и удаление — не потеря, а приведение кода в соответствие с формой.
- **Проверка «переменная используется» обобщается.** Перебор объявлений с требованием
  встретить имя больше одного раза нашёл две из двадцати девяти. Тот же тест на других
  крупных функциях — дешёвый способ находить незавершённый замысел.
- **Проверка по тексту ломается на комментарии.** `assert "LATIN_FREQ" not in script`
  упал на моём же пояснении, зачем карта удалена. Утверждать надо образцы кода
  (`const X`, `X[`), а не имена.
- **Удаление константы ломает чужие обвязки.** Тест копии рецепта извлекал `LATIN_FREQ`
  регулярным выражением и упал на `.group(0)`. Удаляя общий символ, надо искать все места,
  которые его читают, — включая тестовые обвязки.

## 2026-09-12: Короткое замыкание скрывает дефект ровно для одного значения

- **`||` вычисляется слева направо.** `f.form_type === 'solution_im' || form.form_type ===
  …` работает для `solution_im` и падает для всех остальных. Дефект, который проявляется
  не на всех входах, переживает ручной просмотр: кто-то проверил один тип формы и увидел,
  что всё в порядке.
- **Падающая строка выше — и ветка ниже мертва.** `else if(f.form_type === 'granules')` не
  выполнялась никогда, поэтому иконка гранул не рисовалась. Мёртвая ветка — это не
  «лишний код», а признак того, что что-то выше работает не так, как кажется.
- **Необъявленная переменная в JS — ошибка выполнения, а не компиляции.** `form` не
  объявлена нигде, но обнаруживается это только когда до чтения доходит очередь. Статическая
  проверка «в теле функции нет обращений к `form.`» дешевле, чем ждать падения у
  пользователя.
- **Заглушка DOM обязана вести себя как DOM.** Присваивание `innerHTML` убирает потомков.
  Без этого тест читал первый чип вместо последнего и показывал одну иконку для всех
  типов — то есть проверял не то, что нужно, и выглядел как «всё одинаково».
- **grep с исключением может скрыть ровно то, что ищешь.** Фильтр по `f.form_type` убрал
  из выдачи строку, содержащую и `f.form_type`, и `form.form_type` одновременно.
- **Системный перебор лучше поиска по наитию.** Перебор списковых литералов дал три
  находки подряд; точечный просмотр этого файла их не дал.

## 2026-09-12: Совпадение списков проверяется тестом, а не внимательностью

- **Одно понятие в трёх местах расходится обязательно.** «Пероральная форма» была
  определена в личном режиме врача, в основном правиле группировки и в фолбэке. Первые два
  совпадали дословно, третий — нет. Никто этого не заметил, потому что расхождение было
  недостижимо на текущих данных.
- **Недостижимый дефект всё равно дефект.** Он ждёт не исправления данных, а добавления
  новой формы. Тест на совпадение списков стоит дешевле, чем поиск такого дефекта потом.
- **Ужесточение лучше расширения, когда вопрос «можно ли принять внутрь».** Фолбэк, который
  добавляет форму в пероральные «на всякий случай», уже однажды предложил крем как
  суспензию. Список того, что принимать внутрь можно, обязан быть замкнутым.
- **A/B на одной оси, а не по памяти.** Я сравнил старый и новый список на 383 вхождениях
  препарата и получил 0 расхождений. Вчерашнее «28 исключений» считалось по другой оси и
  для сравнения непригодно — числа без указанной оси сравнивать нельзя.
- **Мёртвое утверждение — второй раз.** `assert X or True` снова попало в мой тест и снова
  было удалено до запуска. Правильная реакция — не запомнить «проверять на `or True`»,
  а не писать утверждение, которое не может упасть.

## 2026-09-12: Один инструмент на класс находок, а не на одну категорию

- **Я исправляю собственное решение, принятое час назад.** `db/age_group_report.py` закрывал
  60 находок из 264 и дублировал предикат, который уже есть в валидаторе. Единый ворклист
  на все девять категорий лучше: врачу нужен один список, отсортированный по риску, а не
  девять инструментов.
- **Дублирование предиката — это тот же дефект, что я устранял весь день.** Одна величина,
  вычисляемая в двух местах, рано или поздно разъезжается. Ворклист переписывает предикаты
  валидатора дословно и сверяется с ним тестом по каждой категории.
- **Счётчик предупреждений скрывал стоимость разблокировки.** Валидатор пишет WARN только
  потому, что нозология заблокирована; 174 из 264 находок станут ошибкой сборки, как только
  её откроют. Это надо знать до подписания, поэтому поле `becomes_error_when_unblocked`
  есть у каждой записи.
- **Контекст важнее локализации.** «27 режимов без дозы» бесполезно. Запись с текстом КР
  рядом, возрастной группой и меткой — это уже задача, которую можно выполнить.
- **Проверка уникальности ключа поймала настоящий дефект.** 264 находки давали 250
  уникальных `finding_id`: у находок уровня препарата одна локализация встречается
  несколько раз. Без этой проверки ворклист нельзя было бы использовать для учёта —
  две разные задачи выглядели бы одной.
- **Порядок по клиническому риску.** Режим без дозы и доза не по тому основанию таблетки
  важнее отсутствующего маршрута. Сортировка по алфавиту ставила бы `AGE_GROUP_MISMATCH`
  в начало просто потому, что он первый по буквам.

## 2026-09-11: Счётчик находок — это не задача

- **60 предупреждений в шапке ничего не говорят врачу.** Ни локализации, ни дозы, ни
  вопроса. Ворклист с точным адресом записи и одной формулировкой вопроса превращает
  число в работу.
- **Опасное направление ставится первым.** Режим, адресованному старшему возрасту, чем
  сценарий, грозит дозой выше адресной — 19 из 60. Сортировка по риску важнее сортировки
  по алфавиту.
- **Инструмент не решает.** `purpose: PHYSICIAN_REVIEW_ONLY`, вопрос в каждой записи,
  подсказка как гипотеза. Тот же принцип, что и в очереди блочных связей: инструмент
  вычисляет структурные признаки и останавливается перед клиническим суждением.
- **Одна запись не должна учитываться дважды.** Недопустимое значение `age_group` — это
  нарушение допустимого набора, а не «расхождение адресации». Первая версия моей
  программы клала такую запись в оба списка, и обе цифры становились неправдой.
- **Производный отчёт — в `generated/` и в `.gitignore`.** По образцу
  `source_gate_report.json`: артефакт воспроизводится одной командой, а в репозитории
  остаётся инструмент, а не его вывод.
- **Отчёт обязан сверяться с валидатором.** Тест требует, чтобы на реальной БД оба нашли
  одно и то же число (60 = 60). Иначе ворклист может жить своей жизнью.
