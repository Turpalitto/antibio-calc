...
| — | RC-031 | **status: RETRACTED.** **classification: FALSE POSITIVE / REPORTING ERROR.** Originally claimed a confirmed drug-name misattribution on `assembled_regimens` regimen 5574 (database allegedly said "Азитромицин," source PDF said джозамицин). Re-verification via a machine-generated, hash-verifiable evidence packet (`evidence/regimen_5574_verified_evidence.json`, see `REGIMEN_5574_VERIFICATION_REPORT.md`) proved the claim false: `assembled_regimens.sqlite` (live query, SHA-256 `9f505d08...`), `normalized_regimens.sqlite` (SHA-256 `c7b67354...`), and the source PDF (SHA-256 `b8a525c5...`, page 16, extracted with PyMuPDF) all independently and consistently say `антибиотик=джозамицин`. **root cause:** manual conflation of regimen 5574 with a different, genuine Азитромицин 10 mg/kg regimen (likely `regimen_id=6138`, diagnosis-code match, not confirmed with full certainty) during an early worked-example write-up, compounded by a hand-written "database quote" later presented in a report as if it were captured query output rather than typed prose. **clinical corpus impact: NONE CONFIRMED** — no drug-attribution defect exists in the corpus as a result of this investigation; no corpus-wide audit was triggered since the premise didn't hold. **derived-document impact (corrected, not deleted):** `ROOT_CAUSE_REGISTER.md` (this entry), `PROJECT_STATE.md`, `NEXT_TASK.md`, `AI_LOG.md`, `RC030_EVIDENCE_VALIDATION_REPORT.md`, `RC030_12CASE_REVALIDATION.md`, `RC030_TARGETED_SOURCE_RECOVERY_REPORT.md`, `RC030_DOSE_SEMANTICS_REPORT.md`, `DOSE_SANDBOX_RC030_INTEGRATION_REPORT.md`, `DOSE_CALCULATION_TRACE_SPEC.md`; one corrupted (mojibake, unrelated cause) generated artifact deleted (`dose_verification_sandbox/data/phase6_regimen_5574.json`), see `RC031_RETRACTION_AUDIT.md`. **preventive action:** machine-generated, hash-verifiable evidence packets (`dose_verification_sandbox/evidence_model.py`, `EvidenceBlock`) now required for any future clinical-data claim in a report — `DATABASE_QUERY`/`SOURCE_PDF_EXTRACT`/`GENERATED_CALCULATION` origins require a `source_hash` + `retrieval_command` and reject hand-written content at construction time (`EvidenceIntegrityError`); packet tampering after generation is detectable via a re-derivable `packet_hash`. See `RC031_RETRACTION_AUDIT.md`, `REGIMEN_5574_VERIFICATION_REPORT.md`. | Documentation / Process | — | –/–/–/– | N/A (no pipeline defect to fix) — process/tooling fix only | — | — | **RETRACTED 2026-07-16.** Historical record preserved per policy; not deleted. |
| — | RC-030 | **`assembled_regimens.sqlite`'s `unit` column cannot distinguish per-single-administration from per-day dosing** (e.g. `dose=10, unit="mg/kg"` — no column anywhere states whether 10 mg/kg is the daily total or one administration's amount). Discovered by the Dose Calculation Verification Sandbox (P5.6): all 12 real regimens sampled in the Phase 15 pilot resolved to `calculation_status=BLOCKED/AMBIGUOUS_PERIOD` because of this. Same gap applies to max-dose (no `max_daily_mg`/`max_single_mg` columns) and formulation/concentration (no columns at all). **PARTIALLY CLOSED 2026-07-16** via an additive, source-text-based semantics layer (`dose_verification_sandbox/semantics_parser.py`) that reads `source_quote` for explicit per-day/per-dose signals (Russian+English) instead of relying on the missing schema column — this is a workaround/reconstruction, not a fix to the underlying schema, which still lacks the field. Full-corpus classification of all 2,675 rows: 1,645 (61.5%; 87.6% of the 1,543 `REVIEW_REQUIRED` rows) now resolve to an explicit semantic type; 202 (7.6%) remain genuinely `AMBIGUOUS` (no textual signal); 74 (2.8%) remain `UNPARSED` (compound/malformed unit strings — a separate, still-open extraction defect). Pilot re-run: 10/12 originally-blocked cases now calculate correctly; 2/12 correctly remain blocked. See `RC030_DOSE_SEMANTICS_AUDIT.md`, `RC030_DOSE_SEMANTICS_REPORT.md`, `DOSE_SEMANTICS_MODEL.md`, `DOSE_SEMANTICS_PARSER_SPEC.md`. | Knowledge Model / Extraction | High | Hi/Med/–/Med | Architecture Change (assembly-engine schema: add `denominator_time` and max-dose columns, backfill from `source_quote`) — still the real fix; the semantics layer is a read-only reconstruction on top, not a schema migration | High | Med | **PARTIALLY CLOSED 2026-07-16, then INDEPENDENTLY EVIDENCE-VALIDATED 2026-07-16** — a follow-up validation pass (`RC030_EVIDENCE_VALIDATION_REPORT.md`) independently re-checked the parser (not using its own output as ground truth), found and fixed three additional real bugs (dose-token-location guard for the frequency=1 shortcut; `SUB_DAILY_FREQUENCY` block preventing single-dose-exceeds-daily-dose arithmetic; a gap-measurement bug in the validation's own risk audit), and computed precision with Wilson confidence intervals on a 135-row manual sample. **Verdict: B) PARSER IMPLEMENTED BUT NOT VALIDATED — CALCULATIONS BLOCKED.** No semantic type's Wilson-95%-lower-bound precision reaches the required 99% threshold at this sample size (point estimates were 93–100%, but small-sample statistics cap the provable lower bound around 80% even at a perfect 15/15). `calculation_eligibility` is therefore `BLOCKED` for all 2,675 rows in code (`dose_verification_sandbox/validation_status.py`), not just in documentation — corrected counts: 1,611 `PARSER_CANDIDATE` (60.2%), 236 `AMBIGUOUS`, 74 `UNPARSED`. The underlying schema gap (no `denominator_time` column) remains open; see `RC030_SCHEMA_RESPONSIBILITY_DECISION.md` for the recommended long-term fix (add the column to `assembled_regimens` at the assembly layer). Closing RC-030 fully requires either that schema migration, a much larger validation sample (hundreds per semantic type) to clear the 99% bar, or exhaustive human resolution via the Phase 11 ambiguity workflow. |
